	           ...
	           <c:forEach var="attachmentReference" items="${catalogGroup.attachmentReference}">
				 ...
				 <attachmentRefId><wcf:cdata data="${attachmentRefId}"/></attachmentRefId>
			 	 ...
			 	 <c:forEach var="attribute" items="${catalogGroup.attributes}">
						<c:if test="${attribute.typedKey == 'is_master_category'}">
							<c:choose>
								<c:when test="${attribute.typedValue == 'true'}">
									<c:set var="isMasterCategory" value="true" />
								</c:when>
								<c:otherwise>
								    <c:set var="isMasterCategory" value="false" />
								</c:otherwise>
							</c:choose>
						</c:if>
				</c:forEach>
				<c:choose>
						<c:when test="${isMasterCategory == 'true'}">

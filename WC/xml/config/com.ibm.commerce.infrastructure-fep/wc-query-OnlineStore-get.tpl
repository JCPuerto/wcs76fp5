...
BEGIN_SYMBOL_DEFINITIONS
	...
	COLS:SEOURL_SUMMARY = SEOURL:TOKENNAME,SEOURL_ID
...
BEGIN_ASSOCIATION_SQL_STATEMENT
	name=IBM_SEO_URL_Static_Page_Summary
	base_table=STOREENT
	sql=
		SELECT 
			STOREENT.$COLS:STOREENT_UID$, 
		...
		FROM
              ...
              WHERE
                    STOREENT.STOREENT_ID IN($ENTITY_PKS$) AND 
                    SEOURLKEYWORD.SEOURL_ID IN(SELECT SEOURL.SEOURL_ID FROM SEOURL WHERE SEOURL.TOKENNAME IN(SELECT SEOTOKENUSGTYPE.PRIMARYTOKEN FROM SEOTOKENUSGTYPE 
                    WHERE SEOTOKENUSGTYPE.STOREENT_ID IN ($STOREPATH:view$,0) AND SEOTOKENUSGTYPE.ISSTATIC  = 1)) AND 
                    SEOURLKEYWORD.LANGUAGE_ID IN ($CONTROL:LANGUAGES$) AND 
                    SEOURLKEYWORD.STOREENT_ID = STOREENT.STOREENT_ID AND
...
BEGIN_PROFILE
  ...
  BEGIN_ENTITY
   ...
   associated_sql_statement=IBM_Store_Summary
   associated_sql_statement=IBM_SEO_URL_Static_Page_Summary
   associated_sql_statement=IBM_SEO_URL_Static_Page_Parent_Summary
  END_ENTITY
END_PROFILE

...
BEGIN_SQL_STATEMENT
  ...
  base_table=PX_CDPOOL
	...
	sql=insert into PX_CDPOOL (PX_CDPOOL_ID, STORE_ID, USAGETYPE, CODE, STATUS, TRANSFERABLE) values ( ?PX_CDPOOL_ID?, ?STORE_ID?, ?USAGETYPE?, ?CODE?, ?STATUS?, ?TRANSFERABLE?)
END_SQL_STATEMENT
...
BEGIN_SQL_STATEMENT
  ...
  base_table=PX_CDPOOL
	...
	sql=delete from px_cdpool where px_cdpool_id in (select px_cdpool_id from px_cdpromo where px_promotion_id=?PX_PROMOTION_ID?)
END_SQL_STATEMENT
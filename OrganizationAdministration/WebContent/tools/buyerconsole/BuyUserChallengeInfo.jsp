...
function initializeUserChallengeInfoData()
{
   if ("<%=memberId%>" != null || "<%=memberId%>" != "") {
      if (document.entryForm.<%= ECUserConstants.EC_UREG_CHALLENGEQUESTION %>.value == "") {
          document.entryForm.<%= ECUserConstants.EC_UREG_CHALLENGEQUESTION %>.value  = "<%= bnUser.getAttribute(ECUserConstants.EC_UREG_CHALLENGEQUESTION) %>";
      }
      if (document.entryForm.<%= ECUserConstants.EC_UREG_CHALLENGEANSWER %>.value == "") {
          document.entryForm.<%= ECUserConstants.EC_UREG_CHALLENGEANSWER %>.value    = "<%= bnUser.getAttribute(ECUserConstants.EC_UREG_CHALLENGEANSWER) %>";
      }
   }
}
...
<table border="0" cellpadding="1" cellspacing="0">
  ...
  <td>
      ...
      <input size="30" type="input" name="<%= ECUserConstants.EC_UREG_CHALLENGEQUESTION %>" maxlength="254">
      </label>
  </td>
  <td>
      <label>
      <%=UIUtil.toHTML((String)userWizardNLS.get("userGeneralchAnswer"))%><br>
      <input size="30" type="input" name="<%= ECUserConstants.EC_UREG_CHALLENGEANSWER %>" maxlength="254">
      </label>
  </td>
  </tr>
  ...
  {
  %>
      <input type="hidden" name="<%= ECUserConstants.EC_UREG_CHALLENGEQUESTION %>" />
      <input type="hidden" name="<%= ECUserConstants.EC_UREG_CHALLENGEANSWER %>" />  
  <%

...
function displayAddrItem(num)
{
   ...
   if (addrOrderList[num] == "street")
   {
      ...
      document.write("<i><%=oedb.getAttribute(ECUserConstants.EC_ADDR_ADDRESS1)%></i><BR>");
      document.write("<i><%=oedb.getAttribute(ECUserConstants.EC_ADDR_ADDRESS2)%></i><BR>");
      document.write("<i><%=oedb.getAttribute(ECUserConstants.EC_ADDR_ADDRESS3)%></i>");
   }
   else if (addrOrderList[num] == "city")
   {
      //println("<%=UIUtil.toJavaScript((String)orgEntityNLS.get("city"))%><BR>");
      document.write("<i><%=oedb.getAttribute(ECUserConstants.EC_ADDR_CITY)%></i>");
   }
   else if (addrOrderList[num] == "state")
   {
      //println("<%=UIUtil.toJavaScript((String)orgEntityNLS.get("state"))%><BR>");
      document.write("<i><%=dbAddress.getStateProvDisplayName()%></i>");
   }
   else if (addrOrderList[num] == "country")
   {
     
      //println("<%=UIUtil.toJavaScript((String)orgEntityNLS.get("country"))%><BR>");
      document.write("<i><%=dbAddress.getCountryDisplayName()%></i>");
   }
   else if (addrOrderList[num] == "zip")
   {
      //println("<%=UIUtil.toJavaScript((String)orgEntityNLS.get("zip"))%><BR>");
      document.write("<i><%=oedb.getAttribute(ECUserConstants.EC_ADDR_ZIPCODE)%></i>");
   }
}

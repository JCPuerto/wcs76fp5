...
function loadStringOptionValuesForIncludedMembers () {
		...
		if (userArray != null){
			...
			%>		
					select.options[<%= i %>] = new Option('<%= includedCustomer.getLogonId() %>','<%= includedCustomer.getId() %>');					
			<% 
				}
			}
			%>
...
function loadStringOptionValuesForExcludedMembers () {
		...
		if (userArray != null){
			...
			%>		
					select.options[<%= i %>] = new Option('<%= exludedCustomer.getLogonId() %>','<%= exludedCustomer.getId() %>');					
			<% 
				}
			}
			%>   

...
<frameset rows="10%, *">
  ...
  <frame name="orderAdjustmentContent" title="<%= orderMgmtNLS.get("AdjustmentPage") %>"
    <% if (firstOrderExist && secondOrderExist) { %>
    	src="OrderAdjustmentContentB2B?<%= ECOptoolsConstants.EC_OPTOOL_FIRSTORDER_ID %>=<%= firstOrderId %>&amp;<%= ECOptoolsConstants.EC_OPTOOL_SECONDORDER_ID %>=<%= secondOrderId %>"
	<% } else if (firstOrderExist) { %>
		src="OrderAdjustmentContentB2B?<%= ECOptoolsConstants.EC_OPTOOL_FIRSTORDER_ID %>=<%= firstOrderId %>"
	<% } else {%>
		src="OrderAdjustmentContentB2B?<%= ECOptoolsConstants.EC_OPTOOL_SECONDORDER_ID %>=<%= secondOrderId %>"
    <% } %>

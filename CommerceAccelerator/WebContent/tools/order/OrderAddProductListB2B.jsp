...
function add() {
	...
	if (totalSize > 0) {
		...
		if (itemForm["Type_"+catentryId].value == "ItemBean")
			...
			URLParam.XMLFile = "order.orderAddProductProductDetailsDialogB2B";
		else if (itemForm["Type_"+catentryId].value == "PackageBean")
			URLParam.XMLFile = "order.orderAddProductPackageDetailsDialogB2B";
		else if (itemForm["Type_"+catentryId].value == "BundleBean")
			URLParam.XMLFile = "order.orderAddProductBundleDetailsDialogB2B";
...
<form name="itemListForm">	
   ...
   <br />
	...
	%>
		<tr class="<%= classStr %>">
			<td>
			<input id="<%= labelName %>" type="radio" name="selectedProduct" <%= checked %> value="<%= pList.getProductListData(i).getCatentryId() %>" />
			</td>
			<td>

						...
						<table border="0" cellpadding="2" cellspacing="1">
							...
							<tr>
								<td>							        
		    						...
		    						<input type="hidden" name="forUserId" value="<%=customerId%>" />
						...
						if(filteredPaymentTCInfo!=null && filteredPaymentTCInfo.length>0){
							...
							<tr id="PaymentMethodLayer_<%=i%>" <%if(edp_SelectedIndex == i){%> style="visibility: visible; display: block;"<%} else {%> style="visibility: hidden; display: none;"<%}%>>
								<td>							
									...
									<form name="<%=edp_PIInfo_Form%>" method="post" action="PIAdd">
										<input type="hidden" name="orderId" value="<%=orderId%>" /> 
										<input type="hidden" name="forUserId" value="<%=customerId%>" />
										<input type="hidden" name="clearForUser" value="true" />										
									...
									<table width="100%" border="0" cellpadding="2" cellspacing="1">
											<tr>
												<td valign="middle" id="PaymentMethodsDisplay_TableCell_13_<%=i%>">
													<button style="width:auto" id="PaymentMethodsDisplayButton" name="PaymentMethodsDisplayButton" onclick="submitPIInfo(document.<%=edp_PIInfo_Form%>); return false;" >
														<%=UIUtil.toHTML((String) orderMgmtNLS.get("addPaymentMethod"))%>
													</button>
												</td>
												<td>												
...
<tr>
   ...
   <%       
   		for(int i=0; i<arrayPI.length; i++){
			...
			<form name='<%=edit_Form%>' method='post' action='PIEdit'>
			<input type="hidden" name="orderId"	value="<%=orderId%>"/>
			<input type="hidden" name="forUserId" value="<%=customerId%>" />
			<input type="hidden" name="clearForUser" value="true" />
			<input type="hidden" name="URL"  value="<%=urlPIredirect%>" />	
										...
										<table border="0" cellpadding="2" cellspacing="1">
											<tr>												
												<td valign="middle" id="PaymentMethods_TableCell_33">													
													<button id="updatePaymentMethod" name="updatePaymentMethod" onclick="submitPIInfo(document.<%=edit_Form%>); return false;" <%if(punchoutPayments.contains(info.getPolicyId())){ %>disabled="disabled"<%} %>><%=UIUtil.toHTML((String) orderMgmtNLS.get("updatePaymentMethod"))%></button>
												</td>
												<td valign="middle" id="PaymentMethods_TableCell_34">												
													<button id="removePaymentMethod" name="removePaymentMethod" onclick="removePIInfo(document.<%=edit_Form%>)" <%if(punchoutPayments.contains(info.getPolicyId())){ %>disabled="disabled"<%} %>><%=UIUtil.toHTML((String) orderMgmtNLS.get("removePaymentMethod"))%></button>
												</td>
											</tr>					
										</table>
						  			</td>

...
<input type="hidden" name="workspace" value="<%=request.getParameter("workspace")%>" id="S1">
<input type="hidden" name="taskgrp" value="<%=request.getParameter("taskgroup")%>" id="S1">
<input type="hidden" name="task" value="<%=request.getParameter("task")%>" id="S1">
<TABLE id="StorePreview_Launch_Table_5">  
   ...
   <tr><td>&nbsp;</td></tr>
     ...
     <tr>
          <TD><label for="S5"><INPUT NAME="startYear" VALUE="<%= UIUtil.toJavaScript(startYear) %>" maxlength="4" size="4" id="S5"></label></TD>
	     <TD><label for="S6"><INPUT NAME="startMonth" VALUE="<%= UIUtil.toJavaScript(startMonth) %>" maxlength="2" size="4" id="S6"></label></TD>
	     <TD><label for="S7"><INPUT NAME="startDay" VALUE="<%= UIUtil.toJavaScript(startDay) %>" maxlength="2" size="4" id="S7"></label></TD>
	     <TD><A HREF="javascript:setupDate1();showCalendar(document.previewForm.calImg1)">
	               <IMG SRC="<%=webalias%>images/tools/calendar/calendar.gif" HEIGHT="20"  WIDTH="20" BORDER="0" alt="<%=UIUtil.toJavaScript(calendarText)%>" id=calImg1></A>&nbsp;</TD>
	             

	             <TD>
		<label for="S8"><INPUT NAME="startTime" VALUE="<%= UIUtil.toJavaScript(startTime) %>" maxlength="5" size="4" id="S8"></label>     
       	</TD>
     </tr>
     </table></div></td></tr>
    ...
    <tr><td>&nbsp;</td></tr>
    <TR><TD><LABEL for="S9"><span title="&nbsp;<%= UIUtil.toJavaScript((String)previewNLS.get("inventTXT1")) %>&nbsp;<%= UIUtil.toJavaScript((String)previewNLS.get("inventTXT2")) %>&nbsp;">
    <B><%= UIUtil.toHTML((String)previewNLS.get("ProductTitle")) %></B></LABEL>
    <IMG onclick="javascript:alertDialog('<%= UIUtil.toJavaScript((String)previewNLS.get("inventTXT1")) %>&nbsp;<%= UIUtil.toJavaScript((String)previewNLS.get("inventTXT2")) %>');" SRC="<%=webalias%>images/tools/catalog/info_pop.gif" BORDER="0" alt="<%= UIUtil.toJavaScript((String)previewNLS.get("inventTXT1")) %>&nbsp;<%= UIUtil.toJavaScript((String)previewNLS.get("inventTXT2")) %>&nbsp;<%= UIUtil.toJavaScript((String)previewNLS.get("inventTXT3")) %>">
    </span>
    </TD></TR>

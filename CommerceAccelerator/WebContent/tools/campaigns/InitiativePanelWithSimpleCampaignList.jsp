...
<%@ page language="java"
	...
	com.ibm.commerce.tools.promotions.DiscountCalculationCodeListBean,
...
<%
	...
	DiscountCalculationCodeListBean discountListBean = new DiscountCalculationCodeListBean();
	if (com.ibm.commerce.tools.common.ToolsConfiguration.isComponentEnabled(com.ibm.commerce.discount.rules.DiscountConst.COMPONENT_RULE_BASED_DISCOUNT)) {
		...
		DataBeanManager.activate(discountListBean, request);
		if (discountListBean != null) {
			numberOfDiscount = discountListBean.getListSize();
		}
	}
	...
	for (int i=0; i<numberOfDiscount; i++) {
		...
		try {
			if (com.ibm.commerce.tools.common.ToolsConfiguration.isComponentEnabled(com.ibm.commerce.discount.rules.DiscountConst.COMPONENT_RULE_BASED_DISCOUNT)) {
				...
				// populate the calculation code data bean
				ccdb.setInitKey_calculationCodeId(discountListBean.getCalcode_Id(i));
				DataBeanManager.activate(ccdb, request);

				// get member group from discount details bean by promotion team
				DiscountDetailsBean ddb = new DiscountDetailsBean(discountListBean.getCalcode_Id(i));
				DataBeanManager.activate(ddb, request);

				// set each field for this discount entry
				discountId = discountListBean.getCalcode_Id(i);
				discountName = discountListBean.getCode(i);
				discountUsage = ccdb.getCalculationUsageId();
				discountStoreId = ccdb.getStoreEntityId();
				dtcg = ddb.getMemberGroupNameVector();
			}
		}
		catch (Exception e) {
...
function loadPanelData () {
	with (document.initiativeForm) {
		if (initiativeDataBean != null) {
			...
			//alert(initiativeDataBean.<%= CampaignConstants.ELEMENT_AVAILABLE_DISCOUNT_COLLATERAL %>.length);
			//alert(dumpObject(initiativeDataBean.<%= CampaignConstants.ELEMENT_AVAILABLE_DISCOUNT_COLLATERAL %>));
			//
...
function recreateCollateralArray (selectedCollateralObj, availableCollateralObj, optionBox) {
	...
	for (var i=0; i<optionBox.length; i++) {
		...
		var collateralIndex = searchCollateral(selectedCollateralObj, collateralId);
		...
		if (collateralIndex >= 0) {
			...
			collateralIndex = searchCollateral(availableCollateralObj, collateralId);
			if (collateralIndex >= 0) {
				// duplicate the original collateral object into the array
...
function searchCollateral (collateralList, collateralId) {
...
<table id="WC_InitiativePanel_Table_1">
	<tr>
		<td valign="top" colspan="2" id="WC_InitiativePanel_TableCell_1">
			<label for="selectedPromotion"><%= campaignsRB.get("initiativeWhatPromotionPrompt") %></label>
		</td>
	</tr>
	<tr>
		<td valign="top" id="WC_InitiativePanel_TableCell_2">
			<select name="selectedPromotion" id="selectedPromotion" tabindex="14" class="selectWidth" size="5" onchange="javascript:setButtonContext(this, document.initiativeForm.summaryPromotionButton);updateTargetSegmentSelection();">
			</select>
		</td>
		<td valign="top" id="WC_InitiativePanel_TableCell_3">
			<input type="button" tabindex="15" name="newPromotionButton" value="<%= campaignsRB.get(CampaignConstants.MSG_BUTTON_NEW) %>" onclick="newPromotion();" style="width: 150px" id="newPromotionButton"/><br/>
			<input type="button" tabindex="16" name="summaryPromotionButton" value="<%= campaignsRB.get(CampaignConstants.MSG_BUTTON_SUMMARY) %>" onclick="gotoSummaryPromotionDialog();" style="width: 150px" id="summaryPromotionButton"/>
		</td>
	</tr>
</table>
<table id="WC_InitiativePanel_Table_2">
	<tr>
		<td id="WC_InitiativePanel_TableCell_4">
			<label for="selectedPromotionCollateral"><%= campaignsRB.get(CampaignConstants.MSG_INITIATIVE_WHAT_COLLATERAL_SELECTED) %></label><br/>
			<select name="selectedPromotionCollateral" id="selectedPromotionCollateral" tabindex="17" class="selectWidth" size="10" multiple="multiple" onchange="javascript:updateSloshBuckets(this, document.initiativeForm.removeFromPromotionCollateralSloshBucketButton, document.initiativeForm.availablePromotionCollateral, document.initiativeForm.addToPromotionCollateralSloshBucketButton);initializeSummaryButton(document.initiativeForm.selectedPromotionCollateral, document.initiativeForm.availablePromotionCollateral, document.initiativeForm.summaryPromotionCollateralButton);">
			</select>
		</td>
		<td width="180px" align="center" id="WC_InitiativePanel_TableCell_5">
			<table id="WC_InitiativePanel_Table_12" cellpadding="2" cellspacing="2">
				<tr><td id="WC_InitiativePanel_TableCell_53">
					<input type="button" tabindex="20" name="addToPromotionCollateralSloshBucketButton" value="  <%= campaignsRB.get("GeneralSloshBucketAdd") %>  " style="width: 150px" onclick="addToSelectedPromotionCollateral();" id="addToPromotionCollateralSloshBucketButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_54">
					<input type="button" tabindex="18" name="removeFromPromotionCollateralSloshBucketButton" value="  <%= campaignsRB.get("GeneralSloshBucketRemove") %>  " style="width: 150px" onclick="removeFromSelectedPromotionCollateral();" id="removeFromPromotionCollateralSloshBucketButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_52">
					<input type="button" tabindex="21" name="newPromotionCollateralButton" value="  <%= campaignsRB.get(CampaignConstants.MSG_BUTTON_NEW) %>  " style="width: 150px" onclick="newCollateral();" id="newPromotionCollateralButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_55">
					<input type="button" tabindex="22" name="summaryPromotionCollateralButton" value="  <%= campaignsRB.get(CampaignConstants.MSG_BUTTON_SUMMARY) %>   " style="width: 150px" onclick="gotoSummaryPromotionCollateralDialog()" id="summaryPromotionCollateralButton" />
				</td></tr>
			</table>
		</td>
		<td id="WC_InitiativePanel_TableCell_6">
...
<table id="WC_InitiativePanel_Table_3">
	<tr>
		<td id="WC_InitiativePanel_TableCell_7">
			<label for="selectedCouponCollateral"><%= campaignsRB.get(CampaignConstants.MSG_INITIATIVE_WHAT_COLLATERAL_SELECTED) %></label><br/>
			<select name="selectedCouponCollateral" id="selectedCouponCollateral" tabindex="23" class="selectWidth" size="10" multiple="multiple" onchange="javascript:updateSloshBuckets(this, document.initiativeForm.removeFromCouponCollateralSloshBucketButton, document.initiativeForm.availableCouponCollateral, document.initiativeForm.addToCouponCollateralSloshBucketButton);initializeSummaryButton(document.initiativeForm.selectedCouponCollateral, document.initiativeForm.availableCouponCollateral, document.initiativeForm.summaryCouponCollateralButton);">
			</select>
		</td>
		<td width="180px" align="center" id="WC_InitiativePanel_TableCell_8">
			<table id="WC_InitiativePanel_Table_13" cellpadding="2" cellspacing="2">
				<tr><td id="WC_InitiativePanel_TableCell_57">
					<input type="button" tabindex="26" name="addToCouponCollateralSloshBucketButton" value="  <%= campaignsRB.get("GeneralSloshBucketAdd") %>  " style="width: 150px" onclick="addToSelectedCouponCollateral();" id="addToCouponCollateralSloshBucketButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_58">
					<input type="button" tabindex="24" name="removeFromCouponCollateralSloshBucketButton" value="  <%= campaignsRB.get("GeneralSloshBucketRemove") %>  " style="width: 150px" onclick="removeFromSelectedCouponCollateral();" id="removeFromCouponCollateralSloshBucketButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_56">
					<input type="button" tabindex="27" name="newCouponCollateralButton" value="  <%= campaignsRB.get(CampaignConstants.MSG_BUTTON_NEW) %>  " style="width: 150px" onclick="newCollateral();" id="newCouponCollateralButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_59">
					<input type="button" tabindex="28" name="summaryCouponCollateralButton" value="  <%= campaignsRB.get(CampaignConstants.MSG_BUTTON_SUMMARY) %>   " style="width: 150px" onclick="gotoSummaryCouponCollateralDialog()" id="summaryCouponCollateralButton" />
				</td></tr>
			</table>
		</td>
		<td id="WC_InitiativePanel_TableCell_9">
			<label for="availableCouponCollateral"><%= campaignsRB.get(CampaignConstants.MSG_INITIATIVE_WHAT_COLLATERAL_AVAILABLE) %></label><br/>
			<select name="availableCouponCollateral" id="availableCouponCollateral" tabindex="25" class="selectWidth" size="10" multiple="multiple" onchange="javascript:updateSloshBuckets(this, document.initiativeForm.addToCouponCollateralSloshBucketButton, document.initiativeForm.selectedCouponCollateral, document.initiativeForm.removeFromCouponCollateralSloshBucketButton);initializeSummaryButton(document.initiativeForm.selectedCouponCollateral, document.initiativeForm.availableCouponCollateral, document.initiativeForm.summaryCouponCollateralButton);">
			</select>
		</td>
	</tr>
</table>
...
<table id="WC_InitiativePanel_Table_10">
	<tr>
		<td id="WC_InitiativePanel_TableCell_46">
			<label for="selectedCollateral"><%= campaignsRB.get(CampaignConstants.MSG_INITIATIVE_WHAT_COLLATERAL_SELECTED) %></label><br/>
			<select name="selectedCollateral" id="selectedCollateral" tabindex="58" class="selectWidth" size="10" multiple="multiple" onchange="javascript:updateSloshBuckets(this, document.initiativeForm.removeFromCollateralSloshBucketButton, document.initiativeForm.availableCollateral, document.initiativeForm.addToCollateralSloshBucketButton);initializeSummaryButton(document.initiativeForm.selectedCollateral, document.initiativeForm.availableCollateral, document.initiativeForm.summaryCollateralButton);">
			</select>
		</td>
		<td width="180px" align="center" id="WC_InitiativePanel_TableCell_47">
			<table id="WC_InitiativePanel_Table_14" cellpadding="2" cellspacing="2">
				<tr><td id="WC_InitiativePanel_TableCell_61">
					<input type="button" tabindex="61" name="addToCollateralSloshBucketButton" value="  <%= campaignsRB.get("GeneralSloshBucketAdd") %>  " style="width: 150px" onclick="addToSelectedCollateral();" id="addToCollateralSloshBucketButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_62">
					<input type="button" tabindex="59" name="removeFromCollateralSloshBucketButton" value="  <%= campaignsRB.get("GeneralSloshBucketRemove") %>  " style="width: 150px" onclick="removeFromSelectedCollateral();" id="removeFromCollateralSloshBucketButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_60">
					<input type="button" tabindex="62" name="newCollateralButton" value="  <%= campaignsRB.get(CampaignConstants.MSG_BUTTON_NEW) %>  " style="width: 150px" onclick="newCollateral();" id="newCollateralButton" />
				</td></tr>
				<tr><td id="WC_InitiativePanel_TableCell_63">
					<input type="button" tabindex="63" name="summaryCollateralButton" value="  <%= campaignsRB.get(CampaignConstants.MSG_BUTTON_SUMMARY) %>   " style="width: 150px" onclick="gotoSummaryCollateralDialog()" id="summaryCollateralButton" />
				</td></tr>
			</table>
		</td>
		<td id="WC_InitiativePanel_TableCell_48">
			<label for="availableCollateral"><%= campaignsRB.get(CampaignConstants.MSG_INITIATIVE_WHAT_COLLATERAL_AVAILABLE) %></label><br/>
			<select name="availableCollateral" id="availableCollateral" tabindex="60" class="selectWidth" size="10" multiple="multiple" onchange="javascript:updateSloshBuckets(this, document.initiativeForm.addToCollateralSloshBucketButton, document.initiativeForm.selectedCollateral, document.initiativeForm.removeFromCollateralSloshBucketButton);initializeSummaryButton(document.initiativeForm.selectedCollateral, document.initiativeForm.availableCollateral, document.initiativeForm.summaryCollateralButton);">
			</select>
		</td>
	</tr>
</table>
...
<form name="catalogValidatorForm" action="CampaignInitiativeCatalogValidatorView" target="catalogValidator" method="post" id="catalogValidatorForm">

          ...
          <div id="box" class="my_account generic_error_container">
          	<div id="errorPage">
				...
				<div id="WC_GenericError_5" class="content">
								...
								<c:when test="${errorBean.messageKey != null}">
									<c:choose>
										...
										<c:when test="${errorBean.messageKey eq '_ERR_INVALID_COOKIE'}">
												<span><fmt:message key="INVALID_COOKIE_ERROR" bundle="${storeText}" /></span>
												<br />
												<br />
												<c:url var="LogonFormURL" value="LogonForm">
													<c:param name="storeId" value="${storeId}" />
					...
					<fmt:message key="GENERICERR_TYPE" bundle="${storeText}" />	<c:out value="${errorBean.exceptionType}" />
					<fmt:message key="GENERICERR_KEY" bundle="${storeText}" /> <c:out value="${errorBean.messageKey}"  />
					<fmt:message key="GENERICERR_MESSAGE" bundle="${storeText}" /> <c:out value="${errorBean.message}"  />
					<fmt:message key="GENERICERR_SYSMESSAGE" bundle="${storeText}" /> <c:out value="${errorBean.systemMessage}"  />
					<fmt:message key="GENERICERR_CMD" bundle="${storeText}" /> <c:out value="${errorBean.originatingCommand}"  />
					<fmt:message key="GENERICERR_CORR_ACTION" bundle="${storeText}" /> <c:out value="${errorBean.correctiveActionMessage}"  />

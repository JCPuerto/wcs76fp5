...
<%@ include file="JSTLEnvironmentSetup.jspf"%>
			...
			<c:if test="${!empty param.searchPage}">
				<c:set var="searchPage" value="${param.searchPage}"/>
			</c:if>	
<c:choose>
 ...
 <div id="breadcrumb">
	<c:if test="${categoryPage || productPage || topCategoryPage || compareProductPage || extensionPageWithBCF || searchPage}">
			...
			<c:otherwise>
				...
				<a href="<c:out value='${TopCategoriesDisplayURL}'/>" id="WC_BreadCrumbTrailDisplay_link_2"><fmt:message key="BCT_HOME" bundle="${storeText}" /></a>
							...
							<c:if test="${!empty parent_category_rn && parent_category_rn != top_category}">
								...
								<wcf:url var="ParentCategoryDisplayURL" patternName="CategoryURL" value="Category1">
									<wcf:param name="langId" value="${langId}" />
									<wcf:param name="storeId" value="${WCParam.storeId}" />
									<wcf:param name="catalogId" value="${WCParam.catalogId}" />
								...
								<c:if test="${categoryId != top_category && categoryId != parent_category_rn}">
									...
									<c:choose>
										<c:when test="${(categoryPage && empty parent_category_rn) || (categoryPage && parent_category_rn == categoryId) || (categoryPage && !empty identifier)}">
											<c:choose>
												<c:when test="${!empty finalBreadcrumb}">
													<wcf:url var="SubCategoryDisplayURL" patternName="CanonicalCategoryURL" value="Category3">
														<wcf:param name="langId" value="${langId}" />
														<wcf:param name="storeId" value="${WCParam.storeId}" />
														<wcf:param name="catalogId" value="${WCParam.catalogId}" />
														<wcf:param name="top" value="Y" />
										...
										<c:otherwise>
											<c:choose>
												...
												<c:otherwise>
													...
													<%-- Assume both top_category and parent_category_rn are present --%>
													<c:set var="patternName" value="CategoryURLWithParentCategory"/>
													<c:if test="${empty parent_category_rn && empty top_category}">
														<%-- Assumption is wrong..Both are empty..use different pattern name --%>
														<c:set var="patternName" value="CanonicalCategoryURL"/>
													</c:if>
													<c:if test="${!empty top_category}">
														<%-- top category is not empty...use different pattern name --%>
														<c:set var="patternName" value="CategoryURL"/>
													</c:if>
													<wcf:url var="SubCategoryDisplayURL" patternName="${patternName}" value="Category1">
														<wcf:param name="langId" value="${langId}" />
														<wcf:param name="storeId" value="${WCParam.storeId}" />
														<wcf:param name="catalogId" value="${WCParam.catalogId}" />
														<wcf:param name="categoryId" value="${bctCategory.categoryId}" />
						...
						<c:if test="${searchPage}">							
							<fmt:message key="DIVIDING_BAR" bundle="${storeText}" />
							<span class="breadcrumb_current"><fmt:message key="SEARCH_CATALOG" bundle="${storeText}" /></span>
						</c:if>
			</c:otherwise>
		</c:choose>
			...
			<div id="orderItemDisplay">
				<c:choose>
					<c:when test="${displayCheckoutNavBarOnShoppingCartPage}">
						<span id="step1" class="step_on" title="<fmt:message key='BCT_SHOPPING_CART' bundle='${storeText}'/>"><fmt:message key='BCT_SHOPPING_CART' bundle='${storeText}'/></span>
						<span class="step_arrow"></span>
						<span id="step2" class="step_off" title="<fmt:message key='BCT_ADDRESS' bundle='${storeText}'/>"><fmt:message key='BCT_ADDRESS' bundle='${storeText}'/></span>
						<span class="step_arrow"></span>
						<span id="step3" class="step_off" title="<fmt:message key='BCT_SHIPPING_AND_BILLING' bundle='${storeText}'/>"><fmt:message key='BCT_SHIPPING_AND_BILLING' bundle='${storeText}'/></span>
						<span class="step_arrow"></span>
						<span id="step4" class="step_off" title="<fmt:message key='BCT_ORDER_SUMMARY' bundle='${storeText}'/>"><fmt:message key='BCT_ORDER_SUMMARY' bundle='${storeText}'/></span>
					</c:when>
					<c:otherwise>
					...
					<c:set var="hasBeginIndex" value="false"/>
						...
						<c:if test="${!empty supportedLanguages[1]}">
							...
							<c:if test="${hasPageView == 'true'}">
								<input type="hidden" name="pageView" id="pageView" value="<c:out value='${WCParam.pageView}'/>"/>
							</c:if>
							<c:if test="${hasBeginIndex == 'true'}">
								<input type="hidden" name="beginIndex" id="beginIndex" value="<c:out value='${WCParam.beginIndex}'/>"/>
							</c:if>
				...
				<div id="WC_BreadCrumbTrailDisplay_div_3a" class="breadcrumb_item">
					<flow:ifEnabled  feature="preferredCurrency">
						...
						<c:if test="${!empty supportedCurrencies[1]}">
							<form name="SetCurrencyPreferenceForm" action="${contextAndServletPath}SetCurrencyPreference" method="post" id="SetCurrencyPreferenceForm">
							...
							<input type="hidden" name="langId" value="<c:out value="${WCParam.langId}"  />" id="WC_CachedHeader_FormInput_langId_In_SetCurrencyPreferenceForm_1"/>
							<%-- Get all request parameters and their values --%>

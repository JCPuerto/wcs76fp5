											...
											<c:if test="${!executeExtensionFragment}">
												<c:forEach var="priceEntry" items="${priceMap}">
													<c:if test="${priceEntry.key == catEntryID}">
														<fmt:formatNumber value="${priceEntry.value}" type="currency" currencySymbol="${CurrencySymbolToFormat}" maxFractionDigits="${currencyDecimal}"/><c:out value="${CurrencySymbol}"/>
													</c:if>
												</c:forEach>
											</c:if>
										</td>

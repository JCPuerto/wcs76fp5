...
<c:if test="${WCParam.resultType == 'Brand'}">
	...
	/*
		...
		<c:choose>
			...
			<c:if test="${!empty WCParam.parent_category_rn && WCParam.parent_category_rn != WCParam.categoryId}">
					...
					<c:if test="${type == 'product'}">
						...
						{ 
							...
							"catalogEntry" : { 
												...
												"displayPrice" : "<fmt:formatNumber value='${catalogEntry.calculatedContractPrice.amount}' type='currency' currencySymbol='${CurrencySymbolToFormat}' maxFractionDigits='${currencyDecimal}'/><c:out value="${CurrencySymbol}"/>",
															...
															<c:if test="${definingAttrValue2.usage == 'Defining'}">
																<c:choose>
																	<c:when test="${empty someItemIDs}">
																		<c:set var="someItemIDs" value="${definingAttrValue2.name}_${fn:replace(definingAttrValue2.value.value, searchString, replaceString)}" />
																	</c:when>	
																	<c:otherwise>
																		<c:set var="someItemIDs" value="${someItemIDs},${definingAttrValue2.name}_${fn:replace(definingAttrValue2.value.value, searchString, replaceString)}" />
																  </c:otherwise>	
																</c:choose>
															</c:if>	
														</c:forEach>

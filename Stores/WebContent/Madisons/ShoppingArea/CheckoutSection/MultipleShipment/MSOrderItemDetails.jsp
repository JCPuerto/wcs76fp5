		...
		<tr>
			 <th class="th_align_left_normal <c:out value="${nobottom}"/>" id="MultipleShipping_rowHeader_product<c:out value='${status.count}'/>" abbr="<fmt:message key="Checkout_ACCE_for" bundle="${storeText}"/> ${catEntry.description.name}" width="150">
				...
				<div class="itemspecs" id="WC_MSOrderItemDetails_div_2_<c:out value='${status.count}'/>">
					...
					<span><fmt:message key="CurrentOrder_SKU" bundle="${storeText}"/> <c:out value="${catEntry.partNumber}" escapeXml="false"/></span>
					<br />
						...
						<c:remove var="selectedAttr"/>
							<c:forEach var="attribute" items="${catalogEntry1.catalogEntryAttributes.attributes}" varStatus="status3">
								...
								<c:if test="${!empty selectedAttr && childrenSKUCount > 1}">
									<a class="order_link" id="WC_MSOrderItemDetails_link_1_<c:out value='${status.count}'/>_<c:out value='${status3.count}'/>" href="javascript:;"  onClick="JavaScript:if(this.disabled) return; setUpdateOrderItemIdsForItem(this.id,''); var actionListAttributeMS = new popupActionProperties(); actionListAttributeMS.showAddToCart=false; actionListAttributeMS.showWishList=false; actionListAttributeMS.showProductCompare=false; actionListAttributeMS.showReplaceCartItem=true; showPopup('<c:out value='${parentCatEntry}'/>',event,null,'',actionListAttributeMS,${quickCartOrderItemQuantity}); saveChangeOrderItemId('<c:out value='${itemUniqueId}'/>');"> 
							...
							<flow:ifEnabled feature="SOAWishlist">
								...
								<c:choose>
									<c:when test="${isAjaxCheckout && isAjaxAddToCart}">
										<c:choose>
											<c:when test="${empty defaultWishList}">
													<a href="JavaScript:setCurrentId('WC_MSOrderItemDetails_link_4_${status.count}'); MultipleWishLists.addToNewListAndDelete('<c:out value='${catEntryUniqueId}'/>','<c:out value='${itemUniqueId}'/>');" id="WC_MSOrderItemDetails_link_4_<c:out value='${status.count}'/>">
											...
											<c:otherwise>
												<a href="JavaScript:setCurrentId('WC_MSOrderItemDetails_link_4_${status.count}'); MultipleWishLists.addToListAndDelete('<c:out value='${catEntryUniqueId}'/>','<c:out value='${itemUniqueId}'/>');" id="WC_MSOrderItemDetails_link_4_<c:out value='${status.count}'/>">
													<img src="<c:out value='${jspStoreImgDir}${vfileColor}'/>table_plus_add.png" alt="<c:out value="${txtMoveToWishList}"/>"/>
													<c:out value="${txtMoveToWishList}"/>
												</a>
											</c:otherwise>
										...
										<wcf:url var="OrderItemMove" value="OrderChangeServiceItemDelete">
											...
											<c:choose>
												<c:when test="${empty defaultWishList}">
													...
													<wcf:param name="URL" value="GiftListServiceCreate?URL=${interestItemDisplayURL}" />
												</c:when>
												<c:otherwise>
													<wcf:param name="URL" value="GiftListServiceAddItem?URL=${interestItemDisplayURL}" />
												</c:otherwise>
											</c:choose>
											<wcf:param name="errorViewName" value="OrderShippingBillingView" />
											<wcf:param name="updateable" value="0" />
									...
									<c:when test="${!isAjaxCheckout && isAjaxAddToCart}">
										<wcf:url var="OrderItemMove" value="OrderChangeServiceItemDelete">
											...
											<c:choose>
												<c:when test="${empty defaultWishList}">
													<wcf:param name="name" value="${txtDefaultWishList}" />
													<wcf:param name="URL" value="GiftListServiceCreate?URL=OrderShippingBillingView" />
												</c:when>
												<c:otherwise>
													<wcf:param name="URL" value="GiftListServiceAddItem?URL=OrderShippingBillingView" />
												</c:otherwise>
											</c:choose>												
											<c:if test="${callOrderPrepareOnItemRemove}">
												<wcf:param name="allocate" value="***" />

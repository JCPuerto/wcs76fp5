...
<c:catch>
	...
	<c:if test="${isPersonAddress}">
		...
		<c:if test="${selectedAddressId != -1}" >
			...
			<div class="editAddressLink" id="editAddressLink_<c:out value='${orderItemId}'/>" style="display:block;">
	...
	<%-- Use the value of personalAddressAllowed to hide/show the create/edit address link. --%>
<c:if test="${!empty param.personalAddressAllowed && param.personalAddressAllowed}">
	<div class="newShippingAddressButton" id="newShippingAddressButton_<c:out value='${orderItemId}'/>">
			<a href="JavaScript:setCurrentId('MS_ShipmentAddress_<c:out value='${orderItemId}'/>'); CheckoutHelperJS.addNewShippingAddressForMS('<c:out value='${orderItemId}'/>');CheckoutHelperJS.displayAddressDetailsForMSHelper('<c:out value='${orderItemId}'/>','Shipping');" id="WC_ShippingAddressSelectMultiple_link_3_<c:out value='${orderItemId}'/>">

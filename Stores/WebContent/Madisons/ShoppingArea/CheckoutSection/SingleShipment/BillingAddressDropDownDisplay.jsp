...
<div class="billing_address" id="WC_BillingAddressDropDownDisplay_div_<c:out value='${param.paymentAreaNumber}'/>">
	<p class="title"><label for="billing_address_id_<c:out value='${param.paymentAreaNumber}'/>"><fmt:message key="BILL_BILLING_ADDRESS_CAPS" bundle="${storeText}"/></label>:</p>
		<c:choose>
			<c:when test="${hasValidAddresses}">
				<select class="drop_down" name="billing_address_id" id="billing_address_id_<c:out value='${param.paymentAreaNumber}'/>" onchange="JavaScript:MessageHelper.hideAndClearMessage();CheckoutPayments.displayBillingAddressDetails(this,<c:out value='${param.paymentAreaNumber}'/>,'Billing'); CheckoutPayments.removeCreditCardNumberAndCVV(<c:out value='${param.paymentAreaNumber}'/>, true, true);CheckoutPayments.updatePaymentObject(<c:out value='${param.paymentAreaNumber}'/>, 'billing_address_id');">
		...
		<c:if test="${selectedAddressId != -1 && hasValidAddresses}" >		
			<br/>
			<div id="editBillingAddressLink_<c:out value='${param.paymentAreaNumber}'/>">
					<a href="JavaScript:CheckoutPayments.saveBillingAddressDropDownBoxContextProperties('edit','<c:out value="${param.paymentAreaNumber}"/>');JavaScript:CheckoutHelperJS.editBillingAddress('0','<c:out value="${param.paymentAreaNumber}"/>','<c:out value='${fn:replace(profileshipping,search01,replaceStr01)}'/>','<c:out value='${fn:replace(profilebilling,search01,replaceStr01)}'/>')" id="editBillingAddressLink_a_edit_<c:out value='${param.paymentAreaNumber}'/>">
		...
		<!--  create a new address  -->
			<div id="createNewBillingAddress_<c:out value='${param.paymentAreaNumber}'/>">
					<a href="JavaScript:MessageHelper.hideAndClearMessage();CheckoutPayments.createBillingAddress(<c:out value="${param.paymentAreaNumber}"/>,'Billing');" id="editBillingAddressLink_a_create_<c:out value='${param.paymentAreaNumber}'/>">
						<img src="<c:out value='${jspStoreImgDir}${vfileColor}'/>table_plus_add.png" alt="<fmt:message key="ADDR_CREATE_ADDRESS" bundle="${storeText}"/>" />
						<fmt:message key="ADDR_CREATE_ADDRESS" bundle="${storeText}"/>
					</a>
			</div>

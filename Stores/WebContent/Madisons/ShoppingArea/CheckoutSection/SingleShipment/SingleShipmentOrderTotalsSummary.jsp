...
<div id="total_breakdown">
	<table id="order_total" cellpadding="0" cellspacing="0" border="0" summary="<fmt:message key="ORDER_TOTAL_TABLE_SUMMARY" bundle="${storeText}"/>">
		...
		<tr> 
			...
			<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_2"><fmt:formatNumber value="${order.orderAmount.totalProductPrice.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
		</tr>
	...
	<c:if test="${showTotals eq true}">
		...
		<tr>
			...
			<c:if test="${hasProductDiscount}">
				<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_12"><fmt:formatNumber value="${totalProductDiscount}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
			</c:if>
			<c:if test="${!hasProductDiscount}">
				<td></td>
			</c:if>
										...
										<div class="bodycontent" id="WC_SingleShipmentOrderTotalsSummary_div_4">
											<c:forEach var="adjustment" items="${order.orderAmount.adjustment}">
												<c:if test="${adjustment.displayLevel.name == 'Order'}">
													...
													<span class="tooltip_popup_price"><fmt:formatNumber value="${adjustment.amount.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></span><br />
												</c:if>
											</c:forEach>
										</div>
									</div>
		...
		<c:if test="${param.fromPage != 'shoppingCartDisplay' && param.fromPage != 'pendingOrderDisplay'}">
		  		...
		  		<c:when test="${isBrazilStore}">
					...
					<c:if test="${order.orderAmount.totalSalesTax.value > 0}">
						<tr> <%-- Unusual case so show tax line --%>
							...
							<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_6"><fmt:formatNumber value="${order.orderAmount.totalSalesTax.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
						</tr>	
					</c:if>		  		    
		  		</c:when>
			...
			<tr>
				...
				<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_8"><fmt:formatNumber value="${order.orderAmount.totalShippingCharge.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
			</tr>				
			...
			<tr>
				...
				<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_15"><fmt:formatNumber value="${order.orderAmount.totalShippingTax.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
			</tr>
		</c:if>
		...
		<tr>
			...
			<c:choose>
				...
				<c:otherwise>
					<td class="total_figures breadcrumb_current" id="WC_SingleShipmentOrderTotalsSummary_td_10"><fmt:formatNumber value="${order.orderAmount.grandTotal.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
				</c:otherwise>
			</c:choose>
		</tr>
	</c:if>

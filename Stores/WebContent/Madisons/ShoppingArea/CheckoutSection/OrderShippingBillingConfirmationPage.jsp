                            ...
                            <c:set var="ordStatus" value="${order.orderStatus.status}"/>                            
                                   <c:choose>
                                   		...
                                   		<c:when test="${ordStatus eq 'M' && isPunchoutPaymentPending eq true}">
                                   			<h1 class="breadcrumb_current"><fmt:message key="PUNCHOUT_PAYMENT_ORDER_PENDING_MSG" bundle="${storeText}"/></h1>
                                   		</c:when>
                                   		<c:otherwise>
							              <c:choose>
							              	<c:when test="${WCParam.isQuote eq true}">
					              ...
					              <c:choose>
					              	<c:when test="${WCParam.isQuote eq true}">
	                                   ...
	                                   <p><span class="strong"><fmt:message key="ORD_QUOTE_NUMBER" bundle="${storeText}"/></span>: <span id="WC_OrderShippingBillingConfirmationPage_span_1"><c:out value="${objectId}"/></span></p>
					              	</c:when>
					              	<c:otherwise>
					              		<c:choose>
					              		...
					              		<c:choose>
					              			<c:when test="${ordStatus eq 'I'}">
					              				<p><span class="strong"><fmt:message key="ORD_SCHEDULED_ORDER_NUMBER" bundle="${storeText}"/></span> <span id="WC_OrderShippingBillingConfirmationPage_ScheduledOrderNumber"><c:out value="${objectId}"/></span></p>
					              			</c:when>
					              			<c:when test="${ordStatus eq 'M' && isPunchoutPaymentPending eq true}">
	                                   					<p><span class="strong"><fmt:message key="PUNCHOUT_PAYMENT_PAY_INSTRUCTION_MSG" bundle="${storeText}"/></span></p>
	                                   				</c:when>
					              			<c:otherwise>
					              				<p><span class="strong"><fmt:message key="ORD_ORDER_NUMBER" bundle="${storeText}"/></span>: <span id="WC_OrderShippingBillingConfirmationPage_span_1"><c:out value="${objectId}"/></span></p>
					              			</c:otherwise>
					              		</c:choose>
					              	</c:otherwise>
					              </c:choose>
                                   <c:if test="${!(ordStatus eq 'I' || ordStatus eq 'W')}">
	                                   <c:choose>
							              	<c:when test="${WCParam.isQuote eq true}">
			                                   ...
			                                   <p id="orderDateParagraph"><span class="strong"><fmt:message key="ORD_QUOTE_DATE" bundle="${storeText}"/></span>: <c:out value="${formattedOrderDate}"/></p>
							              	</c:when>
							              	<c:otherwise>
			                                   <c:catch>
			                                   ...
			                                   <p id="orderDateParagraph"><span class="strong"><fmt:message key="ORD_ORDER_DATE" bundle="${storeText}"/></span>: <c:out value="${formattedOrderDate}"/></p>
							              	</c:otherwise>
	                                   </c:choose>
                                   </c:if>
										...
										<c:otherwise>
											<div class="button_footer_line button_footer_line_confirmation_page" id="WC_OrderShipmentDetails_div_31">
												...
												<div class="left left_confirmation_page" id="WC_OrderShippingBillingConfirmationPage_div_32">
													...
													<div class="button_side_message button_side_message_confirmation_page" id="WC_OrderShippingBillingConfirmationPage_div_35">

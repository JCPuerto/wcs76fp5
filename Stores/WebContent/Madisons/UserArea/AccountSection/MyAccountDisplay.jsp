...
<!--[if IE]>
    <link type="text/css" rel="stylesheet" href="${jspStoreImgDir}/css/print_ie.css" media="print"/>
<![endif]-->
<!--[if lte IE 6]>
...
<script type="text/javascript">
	dojo.addOnLoad(function() {
		...
		<fmt:message key="AGE_WARNING_ALERT" bundle="${storeText}" var="AGE_WARNING_ALERT"/>
	...
	if('<wcf:out value="${WCParam.page}" escapeFormat="js"/>'=='orderstatus'){
		if('<wcf:out value="${WCParam.isQuote}" escapeFormat="js"/>'=='true'){
	...
	if('<wcf:out value="${WCParam.loadPage}" escapeFormat="js"/>'=='quickcheckout'){
		dojo.addOnLoad(function() { MyAccountDisplay.loadContentFromURL('checkoutProfile', '${profileFormViewURL}') }); 
	}
	if('<wcf:out value="${WCParam.page}" escapeFormat="js"/>'=='customerlinkwishlist'){
...
wc.render.declareRefreshController({
	...
	,modelChangedHandler: function(message, widget) {
		...
		if (message.actionId == "InterestItemListMessage" ) {
			...
			var maxCharsToDisplay = 25;
			var toEmailAddress = message.recipient.toString();
			
			if (toEmailAddress.length > maxCharsToDisplay) {
				toEmailAddress = toEmailAddress.substr(0, maxCharsToDisplay);
				toEmailAddress = toEmailAddress + '...';
			}

      var wishListSendToMsg = "<fmt:message key="WISHLIST_SENDTO" bundle="${storeText}"><fmt:param value="%0"/></fmt:message>";
			var newWishListSendToMsg = wishListSendToMsg.replace(/%0/, toEmailAddress);
...
--%>
 <script type="text/javascript">
	...
	var myNextURL = '<wcf:out value="${WCParam.nextUrl}" escapeFormat="js"/>';
	if('<wcf:out value="${WCParam.nextUrl}" escapeFormat="js"/>'.length != 0){
		if('<wcf:out value="${WCParam.nextUrl}" escapeFormat="js"/>'.indexOf('AjaxOrderChangeServiceItemAdd') != -1){
			dojo.addOnLoad(function() { invokeItemAdd(); }); 
		} else if('<wcf:out value="${WCParam.nextUrl}" escapeFormat="js"/>'.indexOf('AjaxInterestItemAdd') != -1){
			dojo.addOnLoad(function() { invokeInterestItemAdd(); }); 
		} else if('<wcf:out value="${WCParam.nextUrl}"  escapeFormat="js"/>'.indexOf('AjaxPromotionCodeManage') != -1){
...
<div id="page">
	...
	<script type="text/javascript">
		...
		if('<wcf:out value="${WCParam.page}" escapeFormat="js"/>'=='addressbook'){
			dojo.addOnLoad(function() { 
				MessageHelper.displayStatusMessage(MessageHelper.messages["AB_UPDATE_SUCCESS"]);
			});
	}
		if('<wcf:out value="${WCParam.myAcctParticipation}" escapeFormat="js"/>'=='orgUpdate'){
			dojo.addOnLoad(function() { 
				MessageHelper.displayStatusMessage(MessageHelper.messages["ORG_UPDATE_SUCCESS"]);
			});
		}
		if('<wcf:out value="${WCParam.myAcctParticipation}" escapeFormat="js"/>'=='contractUpdate'){
			dojo.addOnLoad(function() { 

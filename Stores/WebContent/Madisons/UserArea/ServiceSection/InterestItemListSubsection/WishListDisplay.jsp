		...
		<div id="box">
			<div class="my_account_wishlist" id="WC_WishListDisplay_div_18">
				...
				<div dojoType="wc.widget.RefreshArea" id="WishlistDisplay_Widget" controllerId="WishlistDisplay_Controller" role="wairole:region" waistate:live="polite" waistate:atomic="false" waistate:relevant="all">
					...
					<% out.flush(); %>
						<c:choose>
							<c:when test="${!empty WCParam.listId && WCParam.listId != '.'}">
		...
		<div id="right_nav">
					   ...
					   <div id="WC_WishListDisplay_div_14" class="wishlist_side_space">
									<div class="tertiary_button_shadow">
										<div class="tertiary_button left">
											...
											<flow:ifEnabled feature="SOAWishlist">
												<a href="#" onclick="JavaScript:MultipleWishLists.getWishListIdForEmail('SendMsgForm'); MultipleWishLists.checkSOAEmailForm('SendMsgForm','refreshArea');return false;" id="WC_WishListDisplay_links_1"><fmt:message key="SENDWISHLIST" bundle="${storeText}"/></a>
											</flow:ifEnabled>
								   	</div>
								  </div>
					   </div>
			...
			<c:if test="${empty storeError.key}" >
				<c:if test="${!empty WCParam.recipient}">
					...
					<fmt:message key="WISHLIST_SENDTO" bundle="${storeText}"><fmt:param><c:out value="${toEmailAddress}"/></fmt:param></fmt:message>

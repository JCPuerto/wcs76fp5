...
<c:if test="${empty catEntry}">
	<wcbase:useBean id="catEntry" classname="com.ibm.commerce.catalog.beans.CatalogEntryDataBean">
	      <c:set property="catalogEntryID" value="${catalogEntryID}" target="${catEntry}" />
	</wcbase:useBean>
</c:if>
...
<c:set var="catEntryDescription" value="${catalogEntry.description}" scope="request"/>
<c:set var="imageFilePath" value="${catEntry.objectPath}"/>	
						...
						<c:otherwise>
							<div id="popupButton_${prefix}${emsName}_${catEntryIdentifier}" class="main_quickinfo_button">
								...
								<c:set var="buttonLink">
									<a class="strong"  id="QuickInfoButton_${prefix}_${catEntryIdentifier}" href="#"
																onfocus="showPopupButton('${prefix}${emsName}_${catEntryIdentifier}');"
																onclick="javaScript:var actionListImageAcct = new popupActionProperties(); actionListImageAcct.showWishList=${buyable}; actionListImageAcct.showAddToCart=${buyable}; showPopup('${catalogEntryID}',event,null,'popupButton_${prefix}${emsName}_${catEntryIdentifier}',actionListImageAcct); return false;" 
																onkeypress="javaScript:var actionListImageAcct = new popupActionProperties(); actionListImageAcct.showWishList=${buyable}; actionListImageAcct.showAddToCart=${buyable}; showPopup('${catalogEntryID}',event,null,'popupButton_${prefix}${emsName}_${catEntryIdentifier}',actionListImageAcct);" onblur="hidePopupButton('${prefix}${emsName}_${catEntryIdentifier}');" role="wairole:button" waistate:haspopup="true"><fmt:message key="QUICKINFO" bundle="${storeText}"/></a>
						...
						<c:when test="${buyable}" >
							<flow:ifDisabled feature="HidePurchaseButton"> 
								<flow:ifEnabled feature="AjaxAddToCart"> 
									<div id="add2CartPopupButton_${catEntry.catalogEntryID}">
										<c:choose>
											...
											<c:otherwise>
												...
												<c:set var="buttonLink">
																	<a href="javascript:;" onkeypress="javascript: showPopup('${catalogEntryID}',event, null, 'add2CartPopupButton_${catEntry.catalogEntryID}')" onclick="javascript: showPopup('${catalogEntryID}',event, null, 'add2CartPopupButton_${catEntry.catalogEntryID}')" id="WC_CatalogEntryThumbnailDisplayJSPF_${catEntryIdentifier}_links_2" waistate:controls="MiniShoppingCart"><fmt:message key="ADDTOCART" bundle="${storeText}"/></a>
												</c:set>
												<%@ include file="../../ReusableObjects/StoreButton.jspf" %>		
															...
															<span class="button_top">
																<span class="button_bottom">   
																	<a id="QuickInfoButton_${prefix}_${catEntryIdentifier}" href="#"
																		onfocus="showPopupButton('${prefix}${emsName}_${catEntryIdentifier}');"
																		onclick="javaScript:var actionListDetailedAcct = new popupActionProperties(); actionListDetailedAcct.showWishList=${buyable}; actionListDetailedAcct.showAddToCart=${buyable}; showPopup('${catalogEntryID}',event,null,'popupButton_${prefix}${emsName}_${catEntryIdentifier}',actionListDetailedAcct); return false;" 
																		onkeypress="javaScript:var actionListDetailedAcct = new popupActionProperties(); actionListDetailedAcct.showWishList=${buyable}; actionListDetailedAcct.showAddToCart=${buyable}; showPopup('${catalogEntryID}',event,null,'popupButton_${prefix}${emsName}_${catEntryIdentifier}',actionListDetailedAcct);" onblur="hidePopupButton('${prefix}${emsName}_${catEntryIdentifier}');" role="wairole:button" waistate:haspopup="true"><fmt:message key="QUICKINFO" bundle="${storeText}"/></a>
																</span>
															</span>
														</span>		
													</span>
				...
				<td class="desc" id="WC_CatalogEntryThumbnailDisplayJSPF_${catEntryIdentifier}_td_2">
					<c:if test="${pageView == 'detailedMyAccount'}">
						<div class="information">
							<h3><c:out value="${catEntryDescription.name}" escapeXml="false"/></h3>
	...
	<c:when test="${pageView == 'scrollESpot' || pageView == 'scrollSideBarESpot'}">
		<div id="item_8_<c:out value='${emsName}'/>_<c:out value='${catEntryIdentifier}'/>">
								...
								<c:when test="${pageView == 'scrollSideBarESpot'}">
								    ...
								    <c:choose>
								        <c:when test="${!empty image55AttachmentDataBeans[0]}">
								            <c:set var="catEntryImagePath" value="${image55AttachmentDataBeans[0].objectPath}${image55AttachmentDataBeans[0].path}" />
								        </c:when>
								        <c:otherwise>
			...
			<div class="scrollPaneDescription" id="WC_CatalogEntryThumbnailDisplayJSPF_${emsName}_${catEntryIdentifier}_div_31">
				<c:set var="itemDescription" value="${catEntryDescription.name}"/>
				<c:if test="${pageView == 'scrollSideBarESpot'}">
						...
						<c:if test="${fn:length(itemDescription) > maxCharsToDisplay}">
							<c:set var="itemDescription" value="${fn:substring(catEntryDescription.name, 0, maxCharsToDisplay)}..."/>
						</c:if>
				</c:if>
				<a ${ShowProductInNewWindow} href="<c:out value="${catEntryDisplayUrl}" escapeXml="false"/>" title="<c:out value='${catEntryDescription.name}' escapeXml='false'/>" >
					...
					<div class="img" id="WC_CatalogEntryThumbnailDisplayJSPF_${emsName}_${catEntryIdentifier}_div_27">
						...
						<c:choose>
							<c:when test="${!empty image40AttachmentDataBeans[0]}">
								<c:set var="catEntryImagePath" value="${image40AttachmentDataBeans[0].objectPath}${image40AttachmentDataBeans[0].path}" />
							</c:when>
							<c:otherwise>
								<c:set var="catEntryImagePath" value="${jspStoreImgDir}images/NoImageIcon_sm45.jpg" />
							</c:otherwise>
				...
				<div id="rank_${statusCount}_catEntry" class="li rank_catEntry_description" role="gridcell">
					<div class="scrollPaneDescription" id="WC_CatalogEntryThumbnailDisplayJSPF_${emsName}_${catEntryIdentifier}_div_31">
						...
						<c:if test="${fn:length(itemDescription) > maxNumberCharactersAllowedToDisplay}">
							<c:set var="itemDescription" value="${fn:substring(catEntryDescription.name, 0, maxNumberCharactersAllowedToDisplay)}..."/>
						</c:if>
						<a ${ShowProductInNewWindow} href="<c:out value="${catEntryDisplayUrl}" escapeXml="false"/>" >
							<c:out value="${itemDescription}" escapeXml="false"/>
						</a>
									...
									<div id="popupButton_${prefix}${emsName}_${catEntryIdentifier}" class="rightside_quickinfo_button">
											...
											<c:set var="buttonLink">
												<a class="strong"  id="QuickInfoButton_${prefix}_${catEntryIdentifier}" href="#"
																alt="<fmt:message key="QUICKINFO" bundle="${storeText}"/>"
																onfocus="showPopupButton('${prefix}${emsName}_${catEntryIdentifier}');"
																onclick="javaScript:var actionListForSidebar = new popupActionProperties(); actionListForSidebar.showWishList=${buyable}; actionListForSidebar.showAddToCart=${buyable}; showPopup('${catalogEntryID}',event,null,'popupButton_${prefix}${emsName}_${catEntryIdentifier}',actionListForSidebar); return false;" 
																onkeypress="javaScript:var actionListForSidebar = new popupActionProperties(); actionListForSidebar.showWishList=${buyable}; actionListForSidebar.showAddToCart=${buyable}; showPopup('${catalogEntryID}',event,null,'popupButton_${prefix}${emsName}_${catEntryIdentifier}',actionListForSidebar);" onblur="hidePopupButton('${prefix}${emsName}_${catEntryIdentifier}');" role="wairole:button" waistate:haspopup="true"><fmt:message key="QUICKINFO" bundle="${storeText}"/></a>

...
<c:set var="packageShortDesc" value="${fn:replace(packageDataBean.description.shortDescription, search, replaceStr)}"/>
...
<div id="body588">
   ...
   <div id="product">
				...
				<c:choose>
					<c:when test="${!empty packageDataBean.description.fullImage}">
						...
						<img id="productMainImage"
							...
							alt="<c:out value="${packageShortDesc}" escapeXml="false" />"
							title="<c:out value="${packageShortDesc}" escapeXml="false"/>"
		...
		<div class="product_options" id="WC_CachedPackageDisplay_div_3">
			<h1 id="catalog_link"  class="catalog_link"><span itemprop="name"><c:out value="${packageDataBean.description.name}" escapeXml="false"/></span></h1>
				...
				<div id="WC_CachedPackageDisplay_eSpot_div_1" class="promo_price">
					<% out.flush(); %>
					    <c:import url="${jspStoreDir}Snippets/Marketing/ESpot/ESpotNavDisplay.jsp">
			                 ...
			                 <c:param name="currentProductId" value="${packageDataBean.packageID}" />
				...
				<c:when test="${buyable || alwaysShowAddToCart}" >
							...
							<flow:ifEnabled feature="ProductCompare">
								<div class="tertiary_button_shadow">
									<div class="tertiary_button">
										<a href="JavaScript:compareProductJS.Add2CompareAjax('<c:out value='${packageDataBean.packageID}'/>','<c:out value='${productCompareImagePath}'/>', '<c:out value='${catEntryDisplayUrl}'/>','<c:out value='${compareImageDescription}'/>');" id="WC_CachedPackageDisplay_links_4"><fmt:message key="ADDTOCOMPARE" bundle="${storeText}" /></a>
									</div>
								</div>
							</flow:ifEnabled>
								...
								<div class="addToWishListButtonBorder">
									<div class="tertiary_button_shadow" id="addToWishListButton_1">
										<div class="tertiary_button left">
											<a href="javascript: setCurrentId('WC_CachedPackageDisplay_links_5'); categoryDisplayJS.Add2WishListAjaxByID('<c:out value='${packageDataBean.packageID}'/>')" id="WC_CachedPackageDisplay_links_5"><fmt:message key="WISHLIST" bundle="${storeText}" /> </a>
										</div>
									</div>
								</div>								
							</flow:ifEnabled>
					...
					<flow:ifDisabled feature="AjaxAddToCart">				
						 <form name="OrderItemAddForm_<c:out value='${packageDataBean.packageID}'/>" action="OrderChangeServiceItemAdd" method="post" id="OrderItemAddForm_<c:out value='${packageDataBean.packageID}'/>">
							...
							<flow:ifEnabled feature="SOAWishlist">
							<input type="hidden" name="giftListId" value="<c:out value='${giftListId}'/>" id="<c:out value='OrderItemAddForm_${packageDataBean.packageID}_giftListId_${giftListId}'/>"/>
							</flow:ifEnabled>							
						</form>
						<div>
							<span class="primary_button" >
						 ...
						 <div class="features" id="WC_CachedPackageDisplay_div_12">
							 <flow:ifEnabled feature="ProductCompare"> 
								<div class="tertiary_button_shadow">
									<div class="tertiary_button">
										<a href="JavaScript:compareProductJS.Add2CompareAjax('<c:out value='${packageDataBean.packageID}'/>','<c:out value='${productCompareImagePath}'/>', '<c:out value='${catEntryDisplayUrl}'/>','<c:out value='${compareImageDescription}'/>');" id="WC_CachedPackageDisplay_links_7"><fmt:message key="ADDTOCOMPARE" bundle="${storeText}" /></a>
									</div>
								</div>
							</flow:ifEnabled>
							<flow:ifEnabled  feature="wishList">
								<div class="addToWishListButtonBorder">
									<div class="tertiary_button_shadow" id="addToWishListButton_1">
										<div class="tertiary_button left">
											<a id="WC_CachedPackageDisplay_links_8" href="#" onclick="javascript: categoryDisplayJS.Add2WishListByID('<c:out value='${packageDataBean.packageID}'/>',document.getElementById('OrderItemAddForm_<c:out value='${packageDataBean.packageID}'/>'));return false;"><fmt:message key="WISHLIST" bundle="${storeText}" /> </a>
										</div>
									</div>
								</div>								
							</flow:ifEnabled>
		...
		<c:forEach var="compositeProduct" items="${packageDataBean.packagedProducts}" varStatus="iStatus">
				...
				<div class="left package_img" id="WC_CachedPackageDisplay_div_22_<c:out value='${iStatus.count}'/>">
							<c:choose>
				...
				<div class="left package_desc" id="WC_CachedPackageDisplay_div_23_<c:out value='${iStatus.count}'/>">
				...
				<br clear="all" />
				<div class="package_qty" id="WC_CachedPackageDisplay_div_27_<c:out value='${iStatus.count}'/>">  
		...
		<c:forEach var="compositePackage" items="${packageDataBean.packagedPackages}" varStatus="iStatus">
				...
				<div class="left package_img" id="WC_CachedPackageDisplay_div_28_<c:out value='${iStatus.count}'/>">
						<c:choose>
				...
				<div class="left package_desc" id="WC_CachedPackageDisplay_div_29_<c:out value='${iStatus.count}'/>">
				...
				<br clear="all" />
				<div class="package_qty" id="WC_CachedPackageDisplay_div_33_<c:out value='${iStatus.count}'/>">  
				  	...
				  	<c:out value="${packageDataBean.description.shortDescription}" escapeXml="false"/>
						...
						<c:forEach var="attribute2" items="${catentry2.catalogEntryAttributes.attributes}">
							<c:if test="${ attribute2.usage=='Descriptive' }" >	
								...
								<c:choose>
									<c:when test="${ !empty attribute2.extendedValue['Image1']  }" >
										<span id="descAttributeValue_${descAttrCount}"><c:out value="${attribute2.value.value}" escapeXml="false" />&nbsp;<img src="<c:out value="${packageDataBean.objectPath}${attribute2.extendedValue['Image1']}" />" alt="<c:out value="${attribute2.value.value}" />" border="0"/></span><br/>												
									</c:when>
									<c:otherwise >
				  ...
				  <div id="Attachments" href="<c:out value='${CatalogAttachmentURL}'/>" parseOnLoad="false" dojoType="dijit.layout.ContentPane" >
					...
					<flow:ifEnabled  feature="ProductDetailDescListView"> 
						...
						<c:import url="${jspStoreDir}Snippets/Catalog/Attachments/CatalogAttachmentAssetsDisplay.jsp">  
							   ...
							   <c:param name="catalogId" value="${WCParam.catalogId}"/>  
							   <c:param name="langId" value="${langId}"/>  
							   <c:param name="productId" value="${packageDataBean.packageID}"/>  

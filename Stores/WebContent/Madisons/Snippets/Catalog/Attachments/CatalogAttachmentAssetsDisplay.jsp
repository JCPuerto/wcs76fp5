...
<c:if test="${displayAttachment}">
						...
						<c:choose>
						    ...
						    <c:choose>
							    <c:when test="${!empty param.URL}">
									    ...
									    <a href="<c:out value='${param.URL}'/>" id="WC_CatalogAttachmentAssetsDisplay_links_1_<c:out value='${status.count}'/>">
										    <img
	...
	<c:if test="${currentAttachmentCount == 0}">
		<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}" /><br />
	</c:if>	
</c:if>

...
<%--
  *****
  * This JSP file renders the digital wallet display section on the checkout pages and my account page.
  *
...
<c:choose>
	<c:when test="${useDigitalWallet eq true}">
		<%@ include file="DigitalWalletDisplay.jspf" %>
	</c:when>
	<c:otherwise>
							...
							<c:forEach var="issuedCoupon" items="${couponBean.coupons}" varStatus="status1">
								...
								<tr id="CouponWalletTable_tr_3_<c:out value='${status1.count}'/>">
									...
									<td id="CouponWalletTable_td_2_<c:out value='${status1.count}'/>" headers="couponWalletTable_expirationDate" class="couponWalletTable_expirationDate">
										<c:catch>
											<fmt:parseDate parseLocale="${dateLocale}" var="expirationDate" value="${issuedCoupon.expirationDate}" pattern="yyyy-MM-dd HH:mm:ss.SSS" timeZone="GMT"/>
										</c:catch>
										<c:if test="${empty expirationDate}">
											<c:catch>
												<fmt:parseDate parseLocale="${dateLocale}" var="expirationDate" value="${issuedCoupon.expirationDate}" pattern="yyyy-MM-dd HH:mm:ss" timeZone="GMT"/>
											</c:catch>
										</c:if>
										<c:choose>
											...
											<c:otherwise>
												<c:out value="${issuedCoupon.expirationDate}"/>
											</c:otherwise>
										</c:choose>
										&nbsp;
									</td>
							...
							<c:forEach var="usedCoupon" items="${usedCouponBean.coupons}" varStatus="status2">
								...
								<tr id="CouponWalletTable_tr_4_<c:out value='${status1.count}'/>" class="selectedCoupon">
									<td id="CouponWalletTable_td_4_<c:out value='${status2.count}'/>" headers="couponWalletTable_couponName" class="couponWalletTable_couponName">
										...
										<!-- get the short description of the coupon/promotion -->
										<c:out value="${promoDesc}"/> - <span class="strong"><fmt:message key="COUPON_APPLIED" bundle="${storeText}"/></span>&nbsp;
									</td>
									<td id="CouponWalletTable_td_5_<c:out value='${status2.count}'/>" headers="couponWalletTable_expirationDate" class="couponWalletTable_expirationDate">
										<c:catch>
											<fmt:parseDate parseLocale="${dateLocale}" var="expirationDate" value="${usedCoupon.expirationDate}" pattern="yyyy-MM-dd HH:mm:ss.SSS" timeZone="GMT"/>
										</c:catch>
										<c:if test="${empty expirationDate}">
											<c:catch>
												<fmt:parseDate parseLocale="${dateLocale}" var="expirationDate" value="${usedCoupon.expirationDate}" pattern="yyyy-MM-dd HH:mm:ss" timeZone="GMT"/>
											</c:catch>
										</c:if>
										<c:choose>
											<c:when test="${!empty expirationDate}">

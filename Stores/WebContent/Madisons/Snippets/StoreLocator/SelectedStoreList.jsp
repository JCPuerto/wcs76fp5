...
<table id="bopis_table1" tabindex="-1" summary="<fmt:message key='SELECTED_STORES_SUMMARY' bundle='${storeText}' />" cellpadding="0" cellspacing="0" border="0" width="100%">
  ...
  <tr>
    ...
    <td id="WC_SelectedStoreList_td_1_<c:out value='${status.count}'/>" width="5" headers="PhysicalStores_tableCell_0">
        <c:choose>
    ...
    <td id="WC_SelectedStoreList_td_2_<c:out value='${status.count}'/>" width="250" headers="PhysicalStores_tableCell_1">
    <p class="strong"><label for="pickUpStore_${i}"><c:out value="${physicalStores[i].description[0].name}" /></label></p>
    ...
    <c:choose>
      <c:when test="${storeHoursIndex > -1}">
        <td id="WC_SelectedStoreList_td_3_<c:out value='${status.count}'/>" width="160" headers="PhysicalStores_tableCell_2"><c:out value="${physicalStores[i].attribute[storeHoursIndex].displayValue}" escapeXml="false" /></td>
      </c:when>
      <c:otherwise>
        <td id="WC_SelectedStoreList_td_4_<c:out value='${status.count}'/>" width="160" headers="PhysicalStores_tableCell_2"></td>
      </c:otherwise>
    </c:choose>                 

      <c:if test="${fromPage == 'ShoppingCart'}">
    <td id="WC_SelectedStoreList_td_5_<c:out value='${status.count}'/>" class="avail" width="168" headers="PhysicalStores_tableCell_3">
      <c:choose>
    ...
    <td id="WC_SelectedStoreList_td_6_<c:out value='${status.count}'/>" headers="PhysicalStores_tableCell_4">

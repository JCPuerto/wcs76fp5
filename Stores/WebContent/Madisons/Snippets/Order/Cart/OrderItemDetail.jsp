...
<flow:ifDisabled feature="AjaxCheckout">                                 
	...
	<c:forEach var="orderItem" items="${pagorder.orderItem}" varStatus="status">
		...
		<c:choose>
			<%-- TODO.. this check is not needed..we will have 'N' number of items in shopping cart and its impossible
			to have parent categories and top categories for all 'N' number of items.. Not sure why this check was added here.. Karson
			any comments ?? --%>
		...
		<tr>
						...
						<c:forEach var="catalogEntry1" items="${catalogEntriesForAttributes}">
							...
							<c:remove var="selectedAttr"/>
								...
								<a class="order_link" id="WC_OrderItemDetailsf_links_1_<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>" href="javascript:;"  
									onClick="JavaScript: if (this.disabled) return; setUpdateOrderItemIdsForItem(this.id,'<c:out value='${status.count}'/>'); var actionListAttribute = new popupActionProperties(); actionListAttribute.showAddToCart=false; actionListAttribute.showWishList=false; actionListAttribute.showProductCompare=false; actionListAttribute.showReplaceCartItem=true; showPopup('<c:out value='${parentCatEntry}'/>',event,null,'',actionListAttribute,${quickCartOrderItemQuantity}); saveChangeOrderItemId('<c:out value='${orderItem.orderItemIdentifier.uniqueID}'/>');"> 
					...
					<c:if test="${!orderItem.orderItemAmount.freeGift}">
						...
						<flow:ifEnabled feature="AjaxCheckout">
							<a id="WC_OrderItemDetailsf_links_2_<c:out value='${status.count}'/>" href="JavaScript:setCurrentId('WC_OrderItemDetailsf_links_2_<c:out value='${status.count}'/>'); CheckoutHelperJS.deleteFromCart('<c:out value='${orderItem.orderItemIdentifier.uniqueID}'/>');">
								<img src="<c:out value='${jspStoreImgDir}${vfileColor}'/>table_x_delete.png" />
								...
								<c:choose>
									<c:when test="${isAjaxCheckout && isAjaxAddToCart}">
										<c:choose>
											<c:when test="${empty defaultWishList}">
												<p tabindex="0">
													<a id="WC_OrderItemDetailsf_links_4_<c:out value='${status.count}'/>" href="JavaScript:setCurrentId('WC_OrderItemDetailsf_links_4_<c:out value='${status.count}'/>'); MultipleWishLists.addToNewListAndDelete('<c:out value='${orderItem.catalogEntryIdentifier.uniqueID}'/>','<c:out value='${orderItem.orderItemIdentifier.uniqueID}'/>');">
											...
											<c:otherwise>
												<p tabindex="0">
													<a id="WC_OrderItemDetailsf_links_4_<c:out value='${status.count}'/>" href="JavaScript:setCurrentId('WC_OrderItemDetailsf_links_4_<c:out value='${status.count}'/>'); MultipleWishLists.addToListAndDelete('<c:out value='${orderItem.catalogEntryIdentifier.uniqueID}'/>','<c:out value='${orderItem.orderItemIdentifier.uniqueID}'/>');">
														<img src="<c:out value='${jspStoreImgDir}${vfileColor}'/>table_plus_add.png" alt="<c:out value="${txtMoveToWishList}"/>"/>
														<c:out value="${txtMoveToWishList}"/>
													</a>
												</p>
									...
									<c:when test="${!isAjaxCheckout && isAjaxAddToCart}">
										<wcf:url var="OrderItemMove" value="OrderChangeServiceItemDelete">
											...
											<c:choose>
												<c:when test="${param.fromPage != null && param.fromPage == 'pendingOrderDisplay'}">
													<wcf:param name="URL" value="GiftListServiceAddItem?URL=PendingOrderDisplayView?orderId=${pagorder.orderIdentifier.uniqueID}" />
													<wcf:param name="errorViewName" value="PendingOrderDisplayView" />
												</c:when>
												<c:when test="${empty defaultWishList}">
													<wcf:param name="name" value="${txtDefaultWishList}" />
													<wcf:param name="URL" value="GiftListServiceCreate?URL=AjaxOrderItemDisplayView" />
													<wcf:param name="errorViewName" value="AjaxOrderItemDisplayView" />
												</c:when>
												<c:otherwise>
													<wcf:param name="URL" value="GiftListServiceAddItem?URL=AjaxOrderItemDisplayView" />
													<wcf:param name="errorViewName" value="AjaxOrderItemDisplayView" />
												</c:otherwise>
											</c:choose>
											<wcf:param name="beginIndex" value="${beginIndex}" />
			...
			<td id="WC_OrderItemDetailsf_td_3_<c:out value='${status.count}'/>" class="<c:out value="${nobottom}"/> each" headers="shoppingCart_tableCell_each shoppingCart_rowHeader_product<c:out value='${status.count}'/>">
				...
				<span class="price">
					<fmt:formatNumber var="formattedUnitPrice" value="${orderItem.orderItemAmount.unitPrice.price.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/>
					...
					<tr>
						<th colspan="4" class="th_align_left_dotted_top_solid_bottom" abbr="<fmt:message key="Checkout_ACCE_prod_discount" bundle="${storeText}"/> <c:out value="${fn:replace(catEntry.description.name, search, replaceStr)}" escapeXml="false"/>" id="shopcart_rowHeader_discount<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>">
							<div class="itemspecs" id="WC_OrderItemDetailsf_div_3_<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>">
								...
								<a class="discount" href='<c:out value="${DiscountDetailsDisplayViewURL}" />' id="WC_OrderItemDetails_Link_ItemDiscount_1_<c:out value='${status2.count}'/>">

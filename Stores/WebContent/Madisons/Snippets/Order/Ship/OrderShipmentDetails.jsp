		...
		<div class="body" id="WC_OrderShipmentDetails_div_6">
			<div class="order_details_my_account" id="WC_OrderShipmentDetails_div_7">
				<c:if test="${order.orderStatus.status eq 'W'}">
					<c:choose>
						<c:when test="${WCParam.isQuote eq true}">
							<p><span class="strong"><fmt:message key="MO_QUOTE_PENDING_APPROVAL_MESSAGE" bundle="${storeText}"/></span></p>
						</c:when>
						<c:otherwise>
							<p><span class="strong"><fmt:message key="MO_ORDER_PENDING_APPROVAL_MESSAGE" bundle="${storeText}"/></span></p>
						</c:otherwise>
					</c:choose>
				</c:if>
				...
				<p>
					<c:choose>
						<c:when test="${WCParam.isQuote eq true}">
							<span class="strong"><fmt:message key="MO_QUOTE_NUMBER" bundle="${storeText}"/></span>
						</c:when>
						<c:otherwise>
							<c:choose>
								<c:when test="${order.orderStatus.status eq 'I'}">
									<span class="strong"><fmt:message key="ORD_SCHEDULED_ORDER_NUMBER" bundle="${storeText}"/></span>
								</c:when>
								<c:when test="${order.orderStatus.status eq 'M' && isPunchoutPaymentPending eq true}">
									<span class="strong"><fmt:message key="PUNCHOUT_PAYMENT_PAY_INSTRUCTION_MSG" bundle="${storeText}"/></span>
								</c:when>
								<c:otherwise>
									<span class="strong"><fmt:message key="MO_ORDER_NUMBER" bundle="${storeText}"/></span>
								</c:otherwise>
							</c:choose>
						</c:otherwise>
					</c:choose>
				...
				<c:if test="${!(order.orderStatus.status eq 'I' || order.orderStatus.status eq 'W')}">
					<c:choose>
						<c:when test="${WCParam.isQuote eq true}">
							<p><span class="strong"><fmt:message key="MO_QUOTE_DATE" bundle="${storeText}"/></span>
						</c:when>
						<c:otherwise>
							<p><span class="strong"><fmt:message key="MO_ORDER_DATE" bundle="${storeText}"/></span>
						</c:otherwise>
					</c:choose>
					<c:catch>
					...
					<c:choose>
							...
							<c:otherwise>
								<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/></p>
							</c:otherwise>
					</c:choose>			
					...
					<div class="figures" id="WC_OrderShipmentDetails_div_19">
						...
						<c:choose>
							<c:when test="${!empty order.orderAmount.totalProductPrice.value}">
								<fmt:formatNumber value="${order.orderAmount.totalProductPrice.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/>
								</c:when>
							<c:otherwise>
								<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/>
							</c:otherwise>
						...
						<c:choose>
							<c:when test="${!empty order.orderAmount.totalAdjustment.value}">
								<fmt:formatNumber value="${order.orderAmount.totalAdjustment.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/>
								</c:when>
							<c:otherwise>
								<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/>
							</c:otherwise>
						...
						<p class="price">
							...
							<c:when test="${!empty order.orderAmount.totalSalesTax.value}">
								<fmt:formatNumber value="${order.orderAmount.totalSalesTax.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/>
								</c:when>
							<c:otherwise>
								<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/>
							</c:otherwise>
						...
						<c:choose>
							<c:when test="${!empty order.orderAmount.totalShippingCharge.value}">
								<fmt:formatNumber value="${order.orderAmount.totalShippingCharge.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/>
								</c:when>
							<c:otherwise>
								<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/>
							</c:otherwise>
						...
						<c:choose>
							<c:when test="${!empty order.orderAmount.totalShippingTax.value}">
								<fmt:formatNumber value="${order.orderAmount.totalShippingTax.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/>
								</c:when>
							<c:otherwise>
								<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/>
							</c:otherwise>
					    ...
					    <c:when test="${order.orderAmount.grandTotal != null}">
								...
								<c:when test="${!empty order.orderAmount.grandTotal.value}">
									<fmt:formatNumber value="${order.orderAmount.grandTotal.value}" type="currency" maxFractionDigits="${currencyDecimal}" currencySymbol="${CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/>
								</c:when>
								<c:otherwise>
									<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/>
								</c:otherwise>

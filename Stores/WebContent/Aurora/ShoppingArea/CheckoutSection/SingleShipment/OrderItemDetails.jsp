				...
				<div class="itemspecs" id="WC_OrderItemDetails_div_2_<c:out value='${status.count}'/>">
					...
					<c:forEach var="catalogEntry1" items="${catalogEntriesForAttributes}" >
						<c:if test="${catalogEntry1.catalogEntryIdentifier.uniqueID == orderItem.catalogEntryIdentifier.uniqueID}">
							...
							<a class="order_link hover_underline" id="WC_OrderItemDetails_links_1_<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>" href="javaScript: if(!this.disabled){setCurrentId('WC_OrderItemDetails_links_1_<c:out value='${status.count}'/>_<c:out value='${status2.count}'/>');
					...
					<c:if test="${!isFreeGift}">
						<flow:ifEnabled feature="AjaxCheckout">
							<a class="remove_address_link" href="JavaScript:setCurrentId('WC_OrderItemDetails_links_2_<c:out value='${status.count}'/>'); CheckoutHelperJS.deleteFromCart('<c:out value='${itemUniqueId}'/>');" id="WC_OrderItemDetails_links_2_<c:out value='${status.count}'/>">

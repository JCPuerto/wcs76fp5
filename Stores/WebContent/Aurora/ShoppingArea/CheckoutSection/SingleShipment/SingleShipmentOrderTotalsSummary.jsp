...
<div id="total_breakdown">
	...
	<table id="order_total" cellpadding="0" cellspacing="0" border="0" role="presentation">
		...
		<tr> 
			...
			<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_2"><fmt:formatNumber value="${order.orderAmount.totalProductPrice.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
		</tr>
	...
	<c:if test="${showTotals eq true}">
		...
		<tr>
			...
			<c:if test="${hasProductDiscount}">
				<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_12"><fmt:formatNumber value="${totalProductDiscount}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
			</c:if>
			<c:if test="${!hasProductDiscount}">
				<td></td>
			</c:if>
				...
				<c:choose>
					...
					<c:when test="${displayDiscountTooltip}">
						<div id="discountDetailsSection" style="display:none">		
							<span id="discountAcceText" style="display:none">
								...
								<c:forEach var="adjustment" items="${order.orderAmount.adjustment}">
									<c:if test="${adjustment.displayLevel.name == 'Order'}">
										...
										<fmt:formatNumber value="${adjustment.amount.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/>
									</c:if>
								</c:forEach>
							</span>
							...
							<img src="<c:out value='${jspStoreImgDir}${vfileColor}/info.png'/>"  alt="<fmt:message key="ORD_DISCOUNT_ADJUSTMENTS" bundle="${storeText}"/>"/><fmt:message key="DISCOUNT1" bundle="${storeText}"/></span>
											...
											<div class="content_right_border">
												<div class="content">
													<div class="header" id="WC_SingleShipmentOrderTotalsSummary_div_1"> 
														<span id="WC_SingleShipmentOrderTotalsSummary_div_2"><fmt:message key="ORD_DISCOUNT_DETAILS_TITLE" bundle="${storeText}"/></span>
														<div class="clear_float"></div>
													</div>
													<div class="body" id="WC_SingleShipmentOrderTotalsSummary_div_3">
														<c:forEach var="adjustment" items="${order.orderAmount.adjustment}">
															<c:if test="${adjustment.displayLevel.name == 'Order'}">
																<c:out value="${adjustment.description.value}" escapeXml="false"/>&nbsp;
																<span class="tooltip_popup_price"><fmt:formatNumber value="${adjustment.amount.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></span><br />
															</c:if>
														</c:forEach>
													</div>
													<div class="clear_float"></div>
					...
					<c:otherwise>
						<fmt:message key="DISCOUNT1" bundle="${storeText}"/> 
					</c:otherwise>
				</c:choose>
			</td>
			<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_4"><fmt:formatNumber value="${order.orderAmount.totalAdjustment.value - totalProductDiscount}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
		</tr>
		...
		<c:if test="${param.fromPage != 'shoppingCartDisplay' && param.fromPage != 'pendingOrderDisplay'}">
		  		...
		  		<c:when test="${isBrazilStore}">
					...
					<c:if test="${order.orderAmount.totalSalesTax.value > 0}">
						<tr> <%-- Unusual case so show tax line --%>
							...
							<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_6"><fmt:formatNumber value="${order.orderAmount.totalSalesTax.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
						</tr>	
					</c:if>		  		    
		  		</c:when>
			...
			<tr>
				...
				<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_8"><fmt:formatNumber value="${order.orderAmount.totalShippingCharge.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
			</tr>				
			...
			<tr>
				...
				<td class="total_figures" id="WC_SingleShipmentOrderTotalsSummary_td_15"><fmt:formatNumber value="${order.orderAmount.totalShippingTax.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
			</tr>
		</c:if>
		...
		<tr>
			...
			<c:choose>
				<c:when test="${param.fromPage == 'shoppingCartDisplay'}">
					<td class="total_figures breadcrumb_current" id="WC_SingleShipmentOrderTotalsSummary_td_13"><fmt:formatNumber value="${order.orderAmount.totalProductPrice.value + order.orderAmount.totalAdjustment.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
				</c:when>
				<c:otherwise>
					<td class="total_figures breadcrumb_current" id="WC_SingleShipmentOrderTotalsSummary_td_10"><fmt:formatNumber value="${order.orderAmount.grandTotal.value}" type="currency" maxFractionDigits="${env_currencyDecimal}" currencySymbol="${env_CurrencySymbolToFormat}"/><c:out value="${CurrencySymbol}"/></td>
				</c:otherwise>
			</c:choose>
		</tr>
	</c:if>

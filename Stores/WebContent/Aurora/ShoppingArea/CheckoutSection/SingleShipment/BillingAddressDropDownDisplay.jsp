...
<div class="billing_address" id="WC_BillingAddressDropDownDisplay_div_<c:out value='${param.paymentAreaNumber}'/>">
	...
	<fmt:message key="BILL_BILLING_ADDRESS_COLON" bundle="${storeText}"/>
	<c:if test="${hasValidAddresses}"></label></c:if>
		...
		<c:if test="${selectedAddressId != -1 && hasValidAddresses}" >		
			...
			<div id="editBillingAddressLink_<c:out value='${param.paymentAreaNumber}'/>" class="editAddressLink hover_underline">
					<a href="JavaScript:CheckoutPayments.saveBillingAddressDropDownBoxContextProperties('edit','<c:out value="${param.paymentAreaNumber}"/>');JavaScript:CheckoutHelperJS.editBillingAddress('0','<c:out value="${param.paymentAreaNumber}"/>','<c:out value='${fn:replace(profileshipping,search01,replaceStr01)}'/>','<c:out value='${fn:replace(profilebilling,search01,replaceStr01)}'/>');CheckoutHelperJS.setLastAddressLinkIdToFocus('editBillingAddressLink_a_edit_<c:out value='${param.paymentAreaNumber}'/>');" id="editBillingAddressLink_a_edit_<c:out value='${param.paymentAreaNumber}'/>">
		...
		<!--  create a new address  -->
			<div id="createNewBillingAddress_<c:out value='${param.paymentAreaNumber}'/>" class="newShippingAddressButton hover_underline">
					<a href="JavaScript:MessageHelper.hideAndClearMessage();CheckoutPayments.createBillingAddress(<c:out value="${param.paymentAreaNumber}"/>,'Billing');CheckoutHelperJS.setLastAddressLinkIdToFocus('editBillingAddressLink_a_create_<c:out value='${param.paymentAreaNumber}'/>');" id="editBillingAddressLink_a_create_<c:out value='${param.paymentAreaNumber}'/>">
						<img src="<c:out value='${jspStoreImgDir}${vfileColor}'/>table_plus_add.png" alt="" />
						<fmt:message key="ADDR_CREATE_ADDRESS" bundle="${storeText}"/>
					</a>
			</div>

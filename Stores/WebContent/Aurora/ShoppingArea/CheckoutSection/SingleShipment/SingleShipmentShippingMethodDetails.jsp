...
<div class="shipping_method" id="WC_SingleShipmentShippingMethodDetails_div_1">
	<p class="title">
		<label for="singleShipmentShippingMode"><fmt:message key="MO_SHIPPINGMETHOD" bundle="${storeText}"/></label>
	</p>
	...
	<flow:ifEnabled feature="ShippingInstructions">
		...
		<p>
			<span class="checkbox">
				...
				/>
				<span class="text">
					<label for='shippingInstructionsCheckbox'><fmt:message key="SHIP_SHIPPING_INSTRUCTIONS_ADD" bundle="${storeText}"/></label>
				</span>
			</span>
		</p>
		</div>
	...
	<c:if test="${showFutureOrders}">
		...
		<p>
			<span class="checkbox">
				...
				/>
				<span class="text">
					<label for='requestShippingDateCheckbox'><fmt:message key="SHIP_REQUESTED_DATE_ADD" bundle="${storeText}"/></label>
				</span>
			</span>
		</p>
		<div name = "requestShippingDateDiv" id = "requestShippingDateDiv" style="display:<c:out value='${requestShippingDateDivDisplay}'/>">
			...
			<div id="requestedShippingDate_inputField" class="dijitCalendarWidth">
				<input 
					...
					size="6" 
					onchange="JavaScript:setCurrentId(this.id); CheckoutHelperJS.updateRequestedShipDate(this); <c:if test="${!isAjaxCheckOut}">setDirtyFlag()</c:if>" 

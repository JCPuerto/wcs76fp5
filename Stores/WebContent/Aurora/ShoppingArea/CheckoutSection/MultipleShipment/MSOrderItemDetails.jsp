		...
		<tr>
			 <th class="th_align_left_normal <c:out value="${nobottom}"/>" id="MultipleShipping_rowHeader_product<c:out value='${status.count}'/>" abbr="<fmt:message key="Checkout_ACCE_for"  /> ${catEntry.name}" width="225">
				...
				<div class="itemspecs hover_underline" id="WC_MSOrderItemDetails_div_2_<c:out value='${status.count}'/>">
						...
						<c:remove var="selectedAttr"/>
							<c:forEach var="attribute" items="${catalogEntry1.catalogEntryAttributes.attributes}" varStatus="status3">
								...
								<c:if test="${!empty selectedAttr && childrenSKUCount > 1}">
									<a class="order_link" id="WC_MSOrderItemDetails_link_1_<c:out value='${status.count}'/>_<c:out value='${status3.count}'/>" href="javaScript:if(!this.disabled) {setCurrentId('WC_MSOrderItemDetails_link_1_<c:out value='${status.count}'/>_<c:out value='${status3.count}'/>');
					...
					<c:if test="${!isFreeGift}">
						<flow:ifEnabled feature="AjaxCheckout">
							<a class="remove_address_link" href="JavaScript:setCurrentId('WC_MSOrderItemDetails_link_2_${status.count}'); CheckoutHelperJS.deleteFromCart('<c:out value='${itemUniqueId}'/>');" onfocus="javascript:CheckoutHelperJS.setLastFocus(this.id);" id="WC_MSOrderItemDetails_link_2_<c:out value='${status.count}'/>">
			...
			<td class="<c:out value="${nobottom}"/> shippingaddress_nest" width="200" id="WC_MSOrderItemDetails_td_1_<c:out value='${status.count}'/>" headers="MultipleShipments_tableCell_shipAddress MultipleShipping_rowHeader_product<c:out value='${status.count}'/>">
				<div class="shipping_address_nester" id="WC_MSOrderItemDetails_div_3_<c:out value='${status.count}'/>"> 
					...
					<%out.flush();%>
						<c:import url="/${sdb.jspStoreDir}/ShoppingArea/CheckoutSection/MultipleShipment/ShippingAddressSelect.jsp">
			...
			<td class="<c:out value="${nobottom}"/> shippingmethod_nest" id="WC_MSOrderItemDetails_td_2_<c:out value='${status.count}'/>" headers="MultipleShipments_tableCell_shipMethod MultipleShipping_rowHeader_product<c:out value='${status.count}'/>">
				<div class="shipping_method_nested" id="WC_MSOrderItemDetails_div_4_<c:out value='${status.count}'/>">
					<%out.flush();%>
						<c:import url="/${sdb.jspStoreDir}/ShoppingArea/CheckoutSection/MultipleShipment/ShippingMethodDetails.jsp">

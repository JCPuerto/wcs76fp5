...
<script type="text/javascript">
	dojo.addOnLoad(function() { 
		...
		CheckoutHelperJS.initializeShipmentPage('<wcf:out value='${WCParam.forceShipmentType}' escapeFormat='js'/>');
...
<c:catch>
	...
	<input type="hidden" id="shippingOrganizationAddressList" value="${contactOrgAddress}" name="shippingOrganizationAddressList"/>
                     ...
                     <div id="content_wrapper" dojoType="wc.widget.RefreshArea" widgetId="content_wrapper" controllerId="controllerForMainAndAddressDiv" role="main">
                              <!-- Content Start -->
				...
				<div class="main_header" id="WC_MultipleShipmentSummary_div_5">
					...
					<div class="body" id="WC_MultipleShipmentSummary_div_13">
						<div id="shipping">
							...
							<div dojoType="wc.widget.RefreshArea" widgetId="MSOrderItemPagingDisplay" id="MSOrderItemPagingDisplay" controllerId="MSOrderItemPaginationDisplayController" ariaMessage="<fmt:message key="ACCE_Status_Order_Item_List_Updated"/>" ariaLiveId="${ariaMessageNode}" role="region" aria-labelledby="MSOrderItemPagingDisplay_ACCE_Label">
								<%out.flush();%>
				...
				<c:choose>
					...
					<c:when test="${recurringOrderEnabled == 'true' && currentOrderIsRecurringOrder == 'true'}">
							<%out.flush();%>
								<c:import url="${jspStoreDir}ShoppingArea/CheckoutSection/RecurringOrderCheckoutDisplay.jsp">
									<c:param value="true" name="isShippingBillingPage"/>
									<c:param value="${currentOrderId}" name="orderId"/>
								</c:import>
							<%out.flush();%>
					...
					<c:otherwise>
					   ...
					   <div class="content_header" id="WC_ShipmentDisplay_div_26">
							...
							<span class="content_text"><label for="numberOfPaymentMethods"><fmt:message key="BILL_MULTIPLE_BILLING_MESSAGE" bundle="${storeText}"/></label>
                                                        <select class="drop_down_billing" name="numberOfPaymentMethods" id="numberOfPaymentMethods" onchange="JavaScript:CheckoutPayments.setNumberOfPaymentMethods(<c:out value="${numberOfPaymentMethods}"/>,this,'paymentSection')">

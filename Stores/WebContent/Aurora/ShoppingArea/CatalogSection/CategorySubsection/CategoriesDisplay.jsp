...
<c:choose>
	<c:when test ="${!empty WCParam.categoryId}">
		<c:set var="catUniqueId" value="${WCParam.categoryId}"/>
	</c:when>
	<c:otherwise>
		<c:set var="catUniqueId" value="${category.categoryId}"/>
	</c:otherwise>
</c:choose>
...
<title>
	<c:choose>
		...
		<c:otherwise>
			<c:out value="${storeName}"/> - <c:out value="${category.description.name}" escapeXml="false"/> - <fmt:message key="CATEGORY_TITLE" bundle="${storeText}"/>
		</c:otherwise>
	</c:choose>
</title>
...
<%@ include file="CategoriesDisplaySCExt.jspf"%>

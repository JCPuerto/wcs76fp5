...
<c:set var="doneButton"><fmt:message key="PRODUCT_ADD_TO_CART" bundle="${storeText}"/></c:set>
...
<c:choose>
	...
	<c:when test="${!empty WCParam.fromPage && WCParam.fromPage == 'requisitionList'}">
		...
		<c:set var="doneButton"><fmt:message key="SHOPCART_UPDATE" bundle="${storeText}"/></c:set>
	</c:when>
...
<html>
	<head>
		<title><fmt:message key="CONFIGURE" bundle="${storeText}" /></title>

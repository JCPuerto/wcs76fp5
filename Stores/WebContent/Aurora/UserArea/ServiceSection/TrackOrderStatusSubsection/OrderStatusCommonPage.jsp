<%--
 ...
 =================================================================
--%>
<!-- BEGIN OrderStatusCommonPage.jsp -->
							...
							<span class="main_header_text">
								<c:choose>
									...
									<c:when test="${WCParam.isRecurringOrder eq true}">
										<fmt:message key='MA_SCHEDULEDORDERS' bundle='${storeText}'/>
									</c:when>
									<c:when test="${WCParam.isSubscription eq true}">
										<fmt:message key='MA_SUBSCRIPTIONS' bundle='${storeText}'/>
									</c:when>
									<c:otherwise>
										<fmt:message key='MA_ORDER_HISTORY' bundle='${storeText}'/>
									</c:otherwise>
								</c:choose>
							</span>
						</div>
					...
					<div class="body" id="WC_OrderStatusCommonPage_div_6">						
						<% out.flush(); %>
							<c:choose>
								...
								<c:when test="${WCParam.isRecurringOrder eq true}">
									<c:import url="${jspStoreDir}Snippets/Subscription/RecurringOrder/RecurringOrderTableDisplay.jsp" ></c:import>
								</c:when>
								<c:when test="${WCParam.isSubscription eq true}">
									<c:import url="${jspStoreDir}Snippets/Subscription/SubscriptionTableDisplay.jsp" ></c:import>
								</c:when>
								<c:otherwise>

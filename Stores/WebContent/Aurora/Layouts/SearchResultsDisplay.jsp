					...
					<c:when test="${empty redirect}">
						<script type="text/javascript">
							dojo.addOnLoad(function() { 
								...
								dojo.byId("SimpleSearchForm_SearchTerm").value = "<wcf:out value='${WCParam.searchTerm}' escapeFormat='js'/>";
								});
						</script>
...
<c:if test="${empty redirect}">
			<body>
				...
				<%@ include file="../Widgets/ESpot/include/ESpotInfoPopupDisplay.jspf"%>
				<div id="page">
													...
													<div class="widget_title_container_position">
														<div class="widget_title_container">
															<span id="search_results_msg" role="heading" aria-level="1">
																<fmt:message key = "SEARCH_RESULTS_FOR"/> <span class="black">"<c:out value='${originalSearchTerm}'/>"</span>
																(<span id="searchTotalCount"><fmt:message key = "{0}_matches"><fmt:param value="${originalTotalSearchCount}"/></fmt:message></span>)
															</span>
														</div>
													</div>
												</c:if>
														...
														<div class="widget_search_results">
															<div class="results_description">
																<fmt:message key = "SORRY_MESSAGE_{0}">
																	<fmt:param value="${originalSearchTerm}"/>
																</fmt:message>
															</div>
				...
				attributes: "<c:out value="${analyticsEscapedFacetAttributes}"/>"
															...
															<c:if test="${!(empty spellcheck && empty contentspellcheck)}">
																...
																<c:forEach items="${suggesstedKeywordsMap}" varStatus="aStatus">
																	<span><a href="${SpellCheckSearchDisplayViewURL}&searchTerm=${aStatus.current}" class="result">${aStatus.current}</a></span>
																	<span>&nbsp;&nbsp;</span>
																</c:forEach>
															</c:if>
															...
															<div class="item_spacer_10px"></div>
															<span>
																<fmt:message key = "SEARCH_RESULTS_FOR"/> <span class="black">"<c:out value='${searchTerm}'/>"</span>
												...
												<c:if test="${totalCount > 0}">
													<div id="Search_Area_div" class="searchResultSpot">
														<c:if test="${env_inPreview eq 'true'}"><div class="ESpotInfo"><a href="#" onclick="showESpotInfoPopup('ESpotInfo_popup_<c:out value="${espotName}"/>'); return false;"><div class="ESpotInfoIcon"></div></a></div></c:if>

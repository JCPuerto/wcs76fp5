			...
			<c:choose>
				<c:when test="${WCParam.bvauthenticateuser eq 'true'}">
					<c:choose>
						...
						<c:when
							test="${(userType == 'G') && ((fn:containsIgnoreCase(header['User-Agent'],'MSIE 8.0')) 
							|| (fn:containsIgnoreCase(header['User-Agent'],'MSIE 7.0')))}">
							<c:redirect url="${logonFormToProductPage}"></c:redirect>
						</c:when>
						<%-- None IE browsers are forwarded to command which renders review submission page. --%>
						<c:when test="${(userType == 'G')}">
							<c:redirect url="${logonFormToSubmissionPage}"></c:redirect>
						</c:when>
						<c:otherwise>

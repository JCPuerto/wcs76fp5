...
<%@ include file="ext/LanguageCurrency_Data.jsp" %>
<c:if test = "${param.custom_data ne 'true'}">
	<%@ include file="LanguageCurrency_Data.jsp" %>
</c:if>

<%@ include file="ext/LanguageCurrency_UI.jsp" %>
<c:if test = "${param.custom_view ne 'true'}">
	<c:choose>
		...
		<c:when test = "${fn:length(supportedCurrencies) == 1 && fn:length(supportedLanguages) == 1}">
			<%@ include file="LanguageCurrency_UI.jsp" %>
		</c:when>
		<c:otherwise>
			<%@ include file="LanguageCurrency_Popup_UI.jsp" %>
		</c:otherwise>
	</c:choose>
</c:if>

		...
		<c:if test="${!empty invRec.inventoryAvailabilityIdentifier.externalIdentifier.physicalStoreIdentifier}">
			<c:choose>
				<c:when test="${! empty(invRec.availabilityDateTime)}">
					<c:catch>
						<fmt:parseDate parseLocale="${dateLocale}" var="date" value="${invRec.availabilityDateTime}" pattern="yyyy-MM-dd'T'HH:mm:ss.SSS'Z'" timeZone="GMT" dateStyle="long"/>
					</c:catch>
					<c:if test="${empty date}">
						<c:catch>
							<fmt:parseDate parseLocale="${dateLocale}" var="date" value="${invRec.availabilityDateTime}" pattern="yyyy-MM-dd'T'HH:mm:ss'Z'" timeZone="GMT" dateStyle="long"/>
						</c:catch>
					</c:if>

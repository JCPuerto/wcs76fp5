...
<flow:ifEnabled feature="InventoryAvailability">
	<c:set var="catentryId" value="${fn:escapeXml(param.catentryId)}"/>
	...
	<c:choose>
		<c:when test="${param.fromPage == 'productDisplay' || param.fromPage == 'bundleDisplayProduct'}">
			<c:choose>
				...
				<c:otherwise>
					<c:set var="htmlIdentifier" value="${fn:escapeXml(param.catalogEntryID)}"/>
	...
	<script type="text/javascript">
		...
		setNumberOfSKUs(<wcf:out value="${param.numberOfSKUs}" escapeFormat="js"/>);
	...
	<div id="check_store_availability" class="rounded_content_box">
		<div id="WC_CatalogEntryInventoryStatus_div_1" class="contentgrad_header">
			<div id="WC_CatalogEntryInventoryStatus_div_2" class="left_corner"></div>
			<div id="WC_CatalogEntryInventoryStatus_div_3" class="heading"><span class="contentgrad_text"><fmt:message key="CHECK_STORE_AVAIL" bundle="${storeText}"/></span></div>
			<div id="WC_CatalogEntryInventoryStatus_div_4" class="right_corner"></div>
		</div>
		<div id="WC_CatalogEntryInventoryStatus_div_5" class="body588">
			...
			<flow:ifEnabled feature="StoreLocator">
				...
				<c:choose>
					...
					<c:when test="${emptyCookie}">
						<p><a id="WC_CatalogEntryInventoryStatus_links_1" class="bopis_link" href="<c:out value='${StoreLocatorURL}'/>">
				...
				<c:choose>
						...
						<c:choose>
							<c:when test="${param.fromPage == 'productDisplay'}">
								<c:if test="${param.numberOfSKUs > 1}">
									<p id="storeShowAvail_${htmlIdentifier}"><a id="WC_CatalogEntryInventoryStatus_links_2" class="bopis_link" href="#" onclick="JavaScript:var ID=getItemId('entitledItem_${fn:escapeXml(param.catalogEntryID)}'); if (ID == null) MessageHelper.displayErrorMessage(message_no_item); else retrieveInvDetailsFromServer('${catentryId}',ID,'${fn:escapeXml(WCParam.storeId)}','${fn:escapeXml(WCParam.langId)}','${fn:escapeXml(WCParam.catalogId)}', '<c:out value="${jspStoreImgDir}images/"/>', '.gif', event);">
										<fmt:message key="INV_Link_Show_Avail" bundle="${storeText}"/>
									</a></p>
								</c:if>
							</c:when>
							<c:when test="${param.fromPage == 'bundleDisplayProduct'}">
								<p id="storeShowAvail_${htmlIdentifier}"><a id="WC_CatalogEntryInventoryStatus_links_3" class="bopis_link" href="#" onclick="JavaScript:var ID=getItemIdForBundleProduct('${fn:escapeXml(param.catalogEntryID)}'); if (ID == null) MessageHelper.displayErrorMessage(message_no_item); else retrieveInvDetailsFromServer('${catentryId}',ID,'${fn:escapeXml(WCParam.storeId)}','${fn:escapeXml(WCParam.langId)}','${fn:escapeXml(WCParam.catalogId)}', '<c:out value="${jspStoreImgDir}images/"/>', '.gif', event);">
									<fmt:message key="INV_Link_Show_Avail" bundle="${storeText}"/>
								</a></p>
							</c:when>
						</c:choose>
					...
					<c:when test="${!emptyCookie}">
						<p id="storeShowAvail_${htmlIdentifier}"><a id="WC_CatalogEntryInventoryStatus_links_4" class="bopis_link" href="#" onclick="javaScript:retrieveInvDetailsFromServer('${catentryId}','${catentryId}','${fn:escapeXml(WCParam.storeId)}','${fn:escapeXml(WCParam.langId)}','${fn:escapeXml(WCParam.catalogId)}', '<c:out value="${jspStoreImgDir}images/"/>', '.gif', event);">
							<fmt:message key="INV_Link_Show_Avail" bundle="${storeText}"/>
						</a></p>
					</c:when>
				</c:choose>
			...
			<flow:ifDisabled feature="StoreLocator">
				<c:set var="catentryId" value="${fn:escapeXml(param.catalogEntryID)}"/>
				<c:choose>
					<c:when test="${param.fromPage == 'productDisplay'}">
						<c:if test="${param.numberOfSKUs > 1}">
							<p id="storeShowAvail_${htmlIdentifier}"><a id="WC_CatalogEntryInventoryStatus_links_5" class="bopis_link" href="#" onclick="JavaScript:var ID=getItemId('entitledItem_${fn:escapeXml(param.catalogEntryID)}'); if (ID == null) MessageHelper.displayErrorMessage(message_no_item); else retrieveInvDetailsFromServer('${catentryId}',ID,'${fn:escapeXml(WCParam.storeId)}','${fn:escapeXml(WCParam.langId)}','${fn:escapeXml(WCParam.catalogId)}', '<c:out value="${jspStoreImgDir}images/"/>', '.gif', event);">
								<fmt:message key="INV_Link_Show_Avail" bundle="${storeText}"/>
							</a></p>
						</c:if>
					</c:when>
					<c:when test="${param.fromPage == 'bundleDisplayProduct'}">
						<c:if test="${param.numberOfSKUs > 1}">
							<p id="storeShowAvail_${htmlIdentifier}"><a id="WC_CatalogEntryInventoryStatus_links_6" class="bopis_link" href="#" onclick="JavaScript:var ID=getItemIdForBundleProduct('${fn:escapeXml(param.catalogEntryID)}'); if (ID == null) MessageHelper.displayErrorMessage(message_no_item); else retrieveInvDetailsFromServer('${catentryId}',ID,'${fn:escapeXml(WCParam.storeId)}','${fn:escapeXml(WCParam.langId)}','${fn:escapeXml(WCParam.catalogId)}', '<c:out value="${jspStoreImgDir}images/"/>', '.gif', event);">
								<fmt:message key="INV_Link_Show_Avail" bundle="${storeText}"/>
							</a></p>
						</c:if>
					</c:when>
		...
		<div id="WC_CatalogEntryInventoryStatus_div_6" class="footer">
			<div id="WC_CatalogEntryInventoryStatus_div_7" class="left_corner"></div>
			<div id="WC_CatalogEntryInventoryStatus_div_8" class="left"></div>
			<div id="WC_CatalogEntryInventoryStatus_div_9" class="right_corner"></div>
		</div>						 
	</div>	
	<br />
</flow:ifEnabled>

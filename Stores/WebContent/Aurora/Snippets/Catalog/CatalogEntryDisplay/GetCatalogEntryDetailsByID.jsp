...
<%-- variable to determine whether to send only price information in response or not--%>
<c:set var="onlyCatalogEntryPrice" value="${WCParam.onlyCatalogEntryPrice}"/>
...
<c:choose>
	<c:when  test="${catEntry.item}">
		...
		<c:catch var ="noListPriceException">
			<fmt:formatNumber var="listPrice" value="${catEntry.listPrice.amount}" type="currency" currencySymbol="${CurrencySymbolToFormat}" maxFractionDigits="${currencyDecimal}"/>
		</c:catch>
		<c:if test = "${noListPriceException!=null}">
			<c:set var="listPrice" value=""/>
		</c:if>
	...
	<c:when test="${catEntry.dynamicKit}">
		...
		<wcf:getData type="com.ibm.commerce.catalog.facade.datatypes.CatalogNavigationViewType" var="catalogNavigationView" expressionBuilder="getCatalogNavigationCatalogEntryView">
		...
		<c:catch var ="noPriceException">
			<c:set var="catalogIdEntry" value="${catalogEntryView}" />
			<%@ include file="../../../Snippets/Search/CatalogEntrySetPriceDisplay.jspf" %>
			<fmt:formatNumber var="offerPrice" value="${priceString}" type="currency" currencySymbol="${CurrencySymbolToFormat}" maxFractionDigits="${currencyDecimal}"/>
		</c:catch>		
		<c:set var="hasPriceForDK" value="false"/>
		<c:if test="${!empty catalogEntryView.price}">
			<c:set var="hasPriceForDK" value="true"/>
		</c:if>
...
/*
{<c:if test="${onlyCatalogEntryPrice ne 'true'}" >
"catalogEntry": {
	...
	,"hasPriceForDK":${hasPriceForDK},
	"showDynamicKit":${showDynamicKit},
	"isDKConfigurable":${isDKConfigurable},
	"pricePending": "<fmt:message key="PRICE_PENDING" bundle="${storeText}"/>",
	"components":[<c:if test="${!empty catalogEntryView.components}">
						<c:forEach items="${catalogEntryView.components}" var="component" varStatus="componentCounter">
							<fmt:formatNumber var="itemComponentQuantity" value="${component.quantity}" type="number" maxFractionDigits="0"/>
											
							<c:choose>
								<c:when test="${itemComponentQuantity>1}">
									<%-- output order item component quantity in the form of "5 x ComponentName" --%>
									<fmt:message var="txtOrderItemQuantityAndName" key="ITEM_COMPONENT_QUANTITY_NAME" bundle="${storeText}"> 
										<fmt:param><c:out value="${itemComponentQuantity}" escapeXml="false"/></fmt:param>
										<fmt:param><c:out value="${component.catalogEntryView.shortDescription}" escapeXml="false"/></fmt:param>
									</fmt:message>
									"<c:out value="${txtOrderItemQuantityAndName}" />"
								</c:when>
								<c:otherwise>
									"<c:out value="${component.catalogEntryView.shortDescription}" />"
								</c:otherwise>
							</c:choose>
							
							<c:choose>
								<c:when test="${componentCounter.last}"></c:when>
...
"productAttributes":[
	...
	<c:if test="${onlyCatalogEntryPrice eq 'true'}" >
	"catalogEntry": {
		"catalogEntryIdentifier": {
			"uniqueID": "${catEntry.catalogEntryID}",
			"externalIdentifier": {
		...
		"offerPrice": "${offerPrice}${CurrencySymbol}",
		"listPrice": "${listPrice}${CurrencySymbol}",
		"listPriced": "${catEntry.listPriced}",
		<c:if test="${catEntry.itemDataBean.checkNumberOfApplicableContractIds}">
			"priceRange": [
							<c:forEach var="i" begin="0" end="${catEntry.itemDataBean.numberOfApplicableContractIds}" varStatus="appContractIdCounter">
								<c:forEach var="contractPriceRange" items="${catEntry.itemDataBean.applicableContractPriceRanges}" varStatus="priceRangeContractCounter">
										...
										<c:forEach var="priceRange" items="${contractPriceRange.rangePrices}" varStatus="priceRangeCounter">
											<c:choose>
												...
												<c:when test="${!empty priceRange.endingNumberOfUnits}">
													<fmt:formatNumber var="localizedPrice" value="${priceRange.contractPrice.amount}" type="currency" currencySymbol="${CurrencySymbolToFormat}" maxFractionDigits="${currencyDecimal}"/>
													{
													 "startingNumberOfUnits" : "${priceRange.startingNumberOfUnits}",
													 "endingNumberOfUnits" : "${priceRange.endingNumberOfUnits}",
													 "localizedPrice" : "${localizedPrice}${CurrencySymbol}"
													},

<%--
 ...
 =================================================================
--%>
<!-- BEGIN CategoryOnlyResultsDisplay.jsp -->
...
<c:forEach var="count" items="${category.catalogEntryDataBeans}" varStatus="status">
	...
	<c:if test="${skuWithNoParent || (!count.item && !count.dynamicKit) || (count.dynamicKit && showDynamicKit == 'true')}">
...
<div class="body588" id="WC_CategoryOnlyResultsDisplay_div_5">
	...
	<table id="<c:out value='${gridView}'/>" cellpadding="0" cellspacing="0" border="0">
				...
				<c:forEach var="catEntry" items="${category.catalogEntryDataBeans}" varStatus="status">
					...
					<c:if test="${skuWithNoParent || (!catEntry.item && !catEntry.dynamicKit) || (catEntry.dynamicKit && showDynamicKit == 'true')}">
							...
							<c:choose>
								<c:when test="${pageView == 'image'}">
									<td class="item" id="WC_CategoryOnlyResultsDisplay_td_2_<c:out value='${status.count}'/>">
										<div <c:if test="${rowItemCount==0}">class="catEntryThumbnail"</c:if> id="WC_CategoryOnlyResultsDisplay_div_6_<c:out value='${status.count}'/>" <c:if test="${rowItemCount!=0}"> class="container" </c:if>>
...
<!-- END CategoryOnlyResultsDisplay.jsp -->

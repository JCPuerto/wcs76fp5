...
<div class="card_info" id="WC_StandardBankServACH_div_1_${paymentAreaNumber}">
	<input type="hidden" name="paymentTCId" value="<c:out value="${paymentTCId}"/>" id="WC_StandardBankServACH_inputs_1_${paymentAreaNumber}"/>
		...
		<span>
			...
			<c:choose>
				<c:when test="${empty strCheckAccountNumber || strCheckAccountNumber==''}">
					<input aria-required="true" type="text" style="width:142px" name="checkingAccountNumber"  value="<c:out value="${CheckAccount_number}" />" id="checkingAccountNumber_1_${paymentAreaNumber}" onchange="JavaScript:CheckoutPayments.updatePaymentObject(<c:out value='${paymentAreaNumber}'/>, 'checkingAccountNumber');" />  
				</c:when>
				<c:otherwise>
		...
		<span>
			...
			<c:choose>
				<c:when test="${empty strCheckRoutingNumber || strCheckRoutingNumber==''}">
					<input aria-required="true" type="text" style="width:142px" name="checkRoutingNumber" value="<c:out value="${check_routing_number}" />" id="checkRoutingNumber_1_${paymentAreaNumber}" onchange="JavaScript:CheckoutPayments.updatePaymentObject(<c:out value='${paymentAreaNumber}'/>, 'checkRoutingNumber');" /> 
				</c:when>
				<c:otherwise>

...
<div class="card_info" id="WC_StandardAmex_div_1_${paymentAreaNumber}">
	<c:choose>
	...
	<input type="hidden" name="cc_brand" value="<c:out value="${cc_brand}" />" id="WC_StandardAmex_inputs_1_${paymentAreaNumber}"/>
	<input type="hidden" name="paymentTCId" value="<c:out value="${paymentTCId}"/>" id="WC_StandardAmex_inputs_2_${paymentAreaNumber}"/>
	...
	<span class="col1">
		<span class="card_number">
			<span>
				<span class="required-field">*</span><label for="edp_Amex_account_<c:out value='${paymentAreaNumber}' />"><fmt:message key="EDPPaymentMethods_AMEX_CARD_NUMBER" bundle="${storeText}"/></label>
			</span>
			<span>
				<input aria-required="true" onchange="JavaScript:CheckoutPayments.removeCreditCardNumberAndCVV(<c:out value='${paymentAreaNumber}'/>, false, true);CheckoutHelperJS.paymentDataDirty(<c:out value='${paymentAreaNumber}'/>, true);CheckoutPayments.updatePaymentObject(<c:out value='${paymentAreaNumber}'/>, 'account');"" type="text" name="account1" style="width:142px" value="<c:out value="${accountValue}"/>" id="account1_${paymentAreaNumber}"/>
			</span>
		</span>	
		<span>
		...
		<span class="visaCVV2">
			<span id="CVVDetailsSection_${paymentAreaNumber}">		
				<span id="StandardAmex_CVVDetails_AcceText_${paymentAreaNumber}" style="display:none">
				...
				<span id="StandardAmex_CVVDetails_${paymentAreaNumber}" class="order_link" title="<fmt:message key='EDPPaymentMethods_CVV_WHAT_IS' bundle='${storeText}'/><fmt:message key='EDPPaymentMethods_CVV_DEFINITION' bundle='${storeText}'/>" onmouseover="javascript: this.title = '';" onmouseout="javascript: this.title = document.getElementById('StandardAmex_CVVDetails_AcceText_${paymentAreaNumber}').innerHTML;" tabindex="0"><fmt:message key="EDPPaymentMethods_CVV_WHAT_IS" bundle="${storeText}"/></span>
							...
							<div class="content_left_border">
								<div class="content_right_border">
									<div class="content">
										...
										<div id="WC_StandardAmex_div_4_${paymentAreaNumber}" class="body">
											<fmt:message key="EDPPaymentMethods_CVV_DEFINITION" bundle="${storeText}"/>
										</div>
									</div>
								</div>

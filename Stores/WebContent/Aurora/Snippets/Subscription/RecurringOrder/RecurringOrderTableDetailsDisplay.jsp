...
<c:when test="${ShowVerbAllRecurringOrders.recordSetTotal <= 0}">
	<div role="row"><div role="gridcell"><fmt:message key="MO_NOORDERSFOUND" bundle="${storeText}"/></div></div>
</c:when>
<c:otherwise>
...
<div role="columnheader" id="RecurringOrderDetailsDisplayExt_li_header_2" class="li order_scheduled_column"><fmt:message key="SCHEDULE_ORDER_INTERVAL_TITLE" bundle="${storeText}"/></div>
<div role="columnheader" id="RecurringOrderDetailsDisplayExt_li_header_3" class="li next_order_column"><fmt:message key="MO_NEXT_ORDER" bundle="${storeText}"/></div>
<div role="columnheader" id="RecurringOrderDetailsDisplayExt_li_header_4" class="li order_status_column"><fmt:message key="MO_SUBSCRIPTION_STATUS" bundle="${storeText}"/></div>
<div role="columnheader" id="RecurringOrderDetailsDisplayExt_li_header_5" class="li total_price_column"><fmt:message key="TOTAL_PRICE" bundle="${storeText}"/></div>
<div role="columnheader" id="RecurringOrderDetailsDisplayExt_li_header_6" class="li options_column"><span class="spanacce"><fmt:message key="MO_ACCE_BUTTON_COLUMN" bundle="${storeText}"/></span></div>
...
<c:choose>
	...
	<div role="row" id="RecurringOrderDisplay_ul_2_${status.count}" class="ul row">
			...
			<fmt:message key="X_DETAILS" bundle="${storeText}" var="RecurringOrderDetailBreadcrumbLinkLabel"> 
			...
			<div role="gridcell" id="RecurringOrderDetailsDisplayExt_order_number_<c:out value='${status.count}'/>" class="li order_number_column">
				...
				<a href="<c:out value='${OrderDetailUrl1}'/>" class="myaccount_link hover_underline" id="OrderDetailLink_NonAjax_<c:out value='${status.count}'/>"><fmt:message key="DETAILS" bundle="${storeText}"/></a>
			</div>
			...
			<fmt:parseNumber var="timeDifference" value="${(indexTime.time - now.time)/1000}" integerOnly="true"/>
			...
			<c:choose>
				<c:when test="${timeDifference > 0 && timeDifference >= cancelRecurringOrderNoticePeriod1}">
					<fmt:message key="SCHEDULE_ORDER_CANCEL_PER" bundle="${storeText}" var="cancelMessage"/>
				</c:when>
				<c:otherwise>
					<fmt:message key="CANCEL_NOTICE_PERIOD_MSG" bundle="${storeText}" var="cancelMessage"> 
						...
						<c:when test="${!empty order.state}">
							<c:choose>
								<c:when test="${order.state eq 'InActive'}" >
									<fmt:message key="INACTIVE_STATE" bundle="${storeText}"/>
								</c:when>
								<c:when test="${order.state eq 'Active'}" >
									<fmt:message key="ACTIVE_STATE" bundle="${storeText}"/>
								</c:when>
								<c:when test="${order.state eq 'Expired'}" >
									<fmt:message key="EXPIRED_STATE" bundle="${storeText}"/>
								</c:when>
								<c:when test="${order.state eq 'Cancelled'}" >
									<fmt:message key="CANCELLED_STATE" bundle="${storeText}"/>
								</c:when>
								<c:when test="${order.state eq 'Completed'}" >
									<fmt:message key="COMPLETED_STATE" bundle="${storeText}"/>
								</c:when>
								<c:when test="${order.state eq 'Suspended'}" >
									<fmt:message key="SUSPENDED_STATE" bundle="${storeText}"/>
								</c:when>
								<c:when test="${order.state eq 'PendingCancel'}" >
									<fmt:message key="PENDING_CANCEL_STATE" bundle="${storeText}"/>
								</c:when>
							</c:choose>
						</c:when>
						<c:otherwise>
				...
				<div class="option_button"> 
					<div id="showOptions_<c:out value='${status.count}'/>" class="option_button">
						<a href="javascript:void(0);" class="button_primary" onkeypress="javaScript:MyAccountDisplay.showActionsPopup(event,'showOptions_<c:out value='${status.count}'/>','actions_popup_<c:out value='${status.count}'/>', 'actions_popup_widget_recurring_order');" onclick="javaScript:MyAccountDisplay.showActionsPopup(null,'showOptions_<c:out value='${status.count}'/>','actions_popup_<c:out value='${status.count}'/>', 'actions_popup_widget_recurring_order');" width="100%" id="RecurringOrderDetailsDisplayExt_option_button_3b_<c:out value='${status.count}'/>" tabindex="0">
							...
							<div class="button_text"><fmt:message key="MO_ACTIONS" bundle="${storeText}"/><img class="actions_down_arrow" src="<c:out value="${jspStoreImgDir}${vfileColor}transparent.gif"/>" alt="<fmt:message key='DOWN_ARROW_IMAGE' bundle='${storeText}'/>"/></div>
		...
		<div id="actions_popup_<c:out value='${status.count}'/>" style="display:none" >
			<div id="actions_popup_div1_<c:out value='${status.count}'/>" class="actions_popup hover_underline">
				...
				<div id="actions_popup_cancel_<c:out value='${status.count}'/>" class="cancel">
					<a href="javaScript:setCurrentId('RecurringOrder_CancelButton_Ajax_<c:out value='${status.count}'/>'); dijit.byId('actions_popup_widget_recurring_order').hide(); MyAccountDisplay.showPopup('recurring_order',${order.subscriptionIdentifier.uniqueID},'recurringOrderCancelMessage_<c:out value='${status.count}'/>');" width="100%" id="RecurringOrder_CancelButton_Ajax_<c:out value='${status.count}'/>" class="link"><fmt:message key="SCHEDULE_ORDER_CANCEL" bundle="${storeText}"/></a>
				</div>
			</div>
		</div>

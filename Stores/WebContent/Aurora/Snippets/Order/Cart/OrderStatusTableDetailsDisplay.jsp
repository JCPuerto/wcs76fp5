...
<fmt:setBundle basename="${jspStoreDir}storetext_v2" var="storeText_v2" />
...
<c:choose>
	<c:when test="${ShowVerbAllOrdersInThisCategory.recordSetTotal <= 0}">
		<c:choose>
			<c:when test="${param.isQuote eq true}">
				<div role="row"><div role="gridcell"><fmt:message key="MO_NOQUOTESFOUND" bundle="${storeText}"/></div></div>
			</c:when>
			<c:otherwise>
	...
	<c:otherwise>
		...
		<div role="row" id="OrderStatusDetailsDisplayExt_ul_1" class="ul column_heading">
			...
			<c:if test="${isProcessedOrdersTab}">
				...
				<c:choose>
					...
					<c:otherwise>
						<fmt:message key="MA_ORDER_DATE" bundle="${storeText}"/>
					</c:otherwise>
				</c:choose>
				</div>
			</c:if>
			...
			<c:if test="${isProcessedOrdersTab}"><div role="columnheader" id="OrderStatusDetailsDisplayExt_li_header_5" class="li status_column_<c:out value="${columnStyle}"/>"><fmt:message key="MO_SUBSCRIPTION_STATUS" bundle="${storeText}"/></div></c:if>
			...
			<div role="columnheader" id="OrderStatusDetailsDisplayExt_li_header_7" class="li total_price_column_<c:out value="${columnStyle}"/>"><fmt:message key="TOTAL_PRICE" bundle="${storeText}"/></div>
		...
		<c:forEach var="order" items="${allOrdersInThisCategory}" varStatus="status">
			...
			<fmt:message key="X_DETAILS" bundle="${storeText}" var="OrderDetailBreadcrumbLinkLabel"> 
				<fmt:param>
				    <flow:ifEnabled feature="SideBySideIntegration">
			...
			<div role="row" id="OrderStatusDetailsDisplayExt_ul_2_${status.count}" class="ul row">
				...
				<div role="gridcell" id="OrderStatusDetailsDisplayExt_order_number_<c:out value='${status.count}'/>" class="hover_underline li order_number_column_<c:out value="${columnStyle}"/>">
					...
					<br/>
					<a href="<c:out value='${OrderDetailUrl1}'/>" class="myaccount_link" id="WC_OrderStatusDisplay_Link_2b_<c:out value='${status.count}'/>"><fmt:message key="DETAILS" bundle="${storeText}"/></a>
				</div>
				<c:if test="${isProcessedOrdersTab}">					
						...
						<flow:ifEnabled feature="SideBySideIntegration">
					    	<c:choose>
					    		<c:when test="${!empty order.orderStatus.status}">
									...
									<span><fmt:message key="MO_OrderStatus_${ordStatus}" bundle="${storeText_v2}"/></span>
								</c:when>
								<c:otherwise>
									<fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/>								
								</c:otherwise>
							...
							<flow:ifEnabled feature="ScheduleOrder">
									...
									<div id="OrderStatusDetailsDisplayExt_option_button_3_<c:out value='${status.count}'/>" class="option_button"> 
										<a href="#" role="button" class="button_primary" id="OrderStatusDetailsDisplayExt_option_button_link_3_<c:out value='${status.count}'/>" width="100%" onclick="javaScript:setCurrentId('OrderStatusDetailsDisplayExt_option_button_link_3_<c:out value='${status.count}'/>'); MyAccountDisplay.cancelScheduledOrder('<c:out value='${objectId}'/>');">
											<div class="left_border"></div>
											<div class="button_text"><fmt:message key="MO_CancelButton" bundle="${storeText}"/></div>												
											<div class="right_border"></div>
										</a>
									</div>

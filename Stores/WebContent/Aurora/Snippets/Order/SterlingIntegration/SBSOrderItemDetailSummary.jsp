...
<c:if test="${numEntries > pageSize}">
 ...
 <table id="order_details" cellpadding="0" cellspacing="0" border="0" width="100%" summary="<fmt:message key="SHOPCART_TABLE_CONFIRM_SUMMARY" bundle="${storeText}"/>">
	...
	<tr class="nested" id="table_header">
		...
		<th class="avail" id="SingleShipment_tableCell_availability"><fmt:message key="MO_SUBSCRIPTION_STATUS" bundle="${storeText}"/></th>
		<th class="QTY" id="SingleShipment_tableCell_quantity" abbr="<fmt:message key="QUANTITY1" bundle="${storeText}"/>"><fmt:message key="QTY" bundle="${storeText}"/></th>
		<th class="each short" id="SingleShipment_tableCell_unitPrice" abbr="<fmt:message key="UNIT_PRICE" bundle="${storeText}"/>"><fmt:message key="EACH" bundle="${storeText}"/></th>
		<th class="total short" id="SingleShipment_tableCell_totalPrice" abbr="<fmt:message key="TOTAL_PRICE" bundle="${storeText}"/>"><fmt:message key="TOTAL" bundle="${storeText}"/></th>
	</tr>
...
<c:forEach begin="${beginIndex }" end="${endIndex-1 }" var="status">
		...
		<tr id="Product_Line_<c:out value='${status}'/>">
			<th class="th_align_left_normal <c:out value="${nobottom}"/>" id="SingleShipment_rowHeader_product<c:out value='${status}'/>" abbr="<fmt:message key="Checkout_ACCE_for" bundle="${storeText}"/>">
				...
				<div class="itemspecs" id="WC_OrderItemDetailsSummaryf_div_2_<c:out value='${status}'/>">
					...
					<span><fmt:message key="CurrentOrder_SKU" bundle="${storeText}"/> <span id="Catalog_Entry_SKU_<c:out value='${status}'/>"></span></span></br>
...
<div id="HiddenArea_OrderLine_Single" style=display:none>
	<span id="Locale_String_S"><c:out value="${locale}"/></span>
		...
		<span id="Product_Discount_Text"><fmt:message key="Checkout_ACCE_prod_discount" bundle="${storeText}"/></span>	
	</div>

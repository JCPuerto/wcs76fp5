...
<table id="order_details" cellpadding="0" cellspacing="0" border="0" width="100%" summary="<fmt:message key="SHOPCART_TABLE_CONFIRM_SUMMARY_MULTI_SHIP" bundle="${storeText}"/>">
	<tr class="nested">
		...
		<th class="avail" id="MultipleShipments_tableCell_availability"><fmt:message key="MO_SUBSCRIPTION_STATUS" bundle="${storeText}"/></th>
		<th class="QTY" id="MultipleShipments_tableCell_quantity" abbr="<fmt:message key="QUANTITY1" bundle="${storeText}"/>"><fmt:message key="QTY" bundle="${storeText}"/></th>
		<th class="each short" id="MultipleShipments_tableCell_unitPrice" abbr="<fmt:message key="UNIT_PRICE" bundle="${storeText}"/>"><fmt:message key="EACH" bundle="${storeText}"/></th>
		<th class="total short" id="MultipleShipments_tableCell_totalPrice" abbr="<fmt:message key="TOTAL_PRICE" bundle="${storeText}"/>"><fmt:message key="TOTAL" bundle="${storeText}"/></th>
	</tr>
...
<c:forEach begin="${beginIndex}" end="${endIndex-1}" var="status">
			...
			<tr id="Product_Line_<c:out value='${status}'/>">
				 <th class="<c:out value="${nobottom}"/> th_align_left_normal" id="MultipleShipping_rowHeader_product<c:out value='${status}'/>" abbr="<fmt:message key="Checkout_ACCE_for" bundle="${storeText}"/>" width="150">
					...
					<div class="itemspecs" id="WC_MSOrderItemDetailsSummaryf_div_2_<c:out value='${pageSize}'/>">
						...
						<fmt:message key="CurrentOrder_SKU" bundle="${storeText}"/> 
								...
								<span id="Catalog_Entry_SKU_<c:out value='${status}'/>"><fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/></span>		
						<br/>
						<%--
					 ***
					...
					--%>	
						...
						<c:if test="${showDynamicKit eq 'true'}">	
							<div class="top_margin5px" id="Dynamic_Kit_Components_Label_<c:out value='${status}'/>" style=display:none ><fmt:message key="CONFIGURATION" bundle="${storeText}"/></div>
							<p>
						...
						<br/>
							...
							<flow:ifEnabled feature="ShippingInstructions">
							   ...
							   <div id="Shipping_Instruction_Label_<c:out value='${status}'/>" style=display:none>
									...
									<p class="text"><span id="Shipping_Instruction_<c:out value='${status}'/>"><fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/></span></p>
									<br />
								</div>
									...
									<div id="Requested_Shipping_Date_Label_<c:out value='${status}'/>" style=display:none>
										<p>
											...
											<p class="text">
													...
													<span id="Order_Item_Requested_Ship_Date_<c:out value='${status}'/>"><fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/></span>
				...
				<td id="WC_MSOrderItemDetailsSummaryf_td_3_<c:out value='${status}'/>" class="<c:out value="${nobottom}"/> avail" headers="MultipleShipments_tableCell_availability MultipleShipping_rowHeader_product<c:out value='${pageSize}'/>">
							...
							<span id="Order_Item_Status_<c:out value='${status}'/>"><fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/></span>								
				...
				<td id="WC_MSOrderItemDetailsSummaryf_td_4_<c:out value='${status}'/>" class="<c:out value="${nobottom}"/> QTY" headers="MultipleShipments_tableCell_quantity MultipleShipping_rowHeader_product<c:out value='${pageSize}'/>">
					<p class="item-quantity">
									...
									<span id="Order_Item_Quantity_<c:out value='${status}'/>"><fmt:message key="MO_NOT_AVAILABLE" bundle="${storeText}"/></span>
...
<div id="HiddenArea_OrderLine_Single" style=display:none>
		...
		<span id="Product_Discount_Text" style=display:none><fmt:message key="Checkout_ACCE_prod_discount" bundle="${storeText}"/></span>	
	</div>

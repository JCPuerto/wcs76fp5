...
<%--
  ...
  *   - currentProductId    
  *     Product ID that is of interest for an eSpot.
...
<% 
   ...
   String productId = request.getParameter("currentProductId");
   if (productId != null) {
...
<c:choose>							
	<c:when test="${isBrazilStore && !empty productId }">
		<!-- for Brazil store, make the Product ID part of the div -->
		<c:set var="WC_RightSidebarESpotDisplay_div_1_ID" value="WC_RightSidebarESpotDisplay_${productId}_div_1_${marketingSpotDatas.marketingSpotIdentifier.uniqueID}"/>
		<c:set var="WC_RightSidebarESpotDisplay_div_2_ID" value="WC_RightSidebarESpotDisplay_${productId}_div_2_${marketingSpotDatas.marketingSpotIdentifier.uniqueID}"/>
	</c:when>
	<c:otherwise>
		<c:set var="WC_RightSidebarESpotDisplay_div_1_ID" value="WC_RightSidebarESpotDisplay_div_1_${marketingSpotDatas.marketingSpotIdentifier.uniqueID}"/>
		<c:set var="WC_RightSidebarESpotDisplay_div_2_ID" value="WC_RightSidebarESpotDisplay_div_2_${marketingSpotDatas.marketingSpotIdentifier.uniqueID}"/>
	</c:otherwise>
</c:choose>
...
<div class="ESpotInfo" id="WC_RightSidebarESpotInfo_<c:out value='${marketingSpotDatas.marketingSpotIdentifier.uniqueID}'/>"><a href="#" onclick="showESpotInfoPopup('ESpotInfo_popup_<c:out value="${emsName}"/>'); return false;"><div class="ESpotInfoIcon"></div></a></div>
...
<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}" varStatus="status">
    <c:if test='${marketingSpotData.dataType eq "CatalogGroup"}'>                
        ...
        <div class="<c:out value="${param.adclass}"/>" id="<c:out value='${param.adclass}_${currentCategoryCount}'/>">
            ...
            <p class="<c:out value="${categoryname_class}"/>">
...
<c:forEach var="marketingSpotData" items="${marketingSpotDatas.baseMarketingSpotActivityData}" varStatus="status2">
		...
		<c:set var="catEntry" value="${catEntry}" scope="request"/>
   		...
   		<c:if test="${(catEntry.dynamicKit && showDynamicKit) || !catEntry.dynamicKit}">        
		    ...
		    <%-- Coremetrics tag --%>
		 	<flow:ifEnabled feature="Analytics">									 			
				<cm:campurl espotData="${marketingSpotDatas}" id="ClickInfoURL" url="${requestScope.ClickInfoURL}" 
		 		initiative="${marketingSpotData.activityIdentifier.uniqueID}" 
		 		name="${requestScope.catEntryDescription.name}" />
			</flow:ifEnabled>
		    <%-- Coremetrics tag --%>					
...
<c:if test="${currentProductCount != 0}">
    ...
    <c:choose>    
        <c:when test="${param.widgetIconStyle != 'top' && (param.showWidget == 'true' || param.showFeed == 'true') && numCatalogEntries != 0 && numContent == 0}">
            <div class="sidebar_ad_remote_widget_product">
                <%out.flush();%>
                <c:import url="${jspStoreDir}Snippets/Marketing/ESpot/RemoteWidgetButtons.jsp">
                    <c:param name="showWidget" value="${param.showWidget}" />
                    <c:param name="showFeed" value="${param.showFeed}" />
        ...
        <c:otherwise>
            <div class="footer" id="WC_RightSidebarESpotDisplay_div_5"></div>
        </c:otherwise>
    </c:choose>
    </div>
</c:if>
            ...
            <c:when test="${marketingSpotData.marketingContent.marketingContentFormat.name == 'File'}">              
                ...
                <c:choose>
                    <c:when test="${(marketingSpotData.marketingContent.mimeType eq 'image') || (marketingSpotData.marketingContent.mimeType eq 'images')}">        
                        ...
                        <img id="WC_RightSidebarESpotDisplay_img_3_<c:out value='${status3.count}'/>"
                        ...
                        <c:choose>
                            ...
                            <c:otherwise>
                                <a id="WC_RightSidebarESpotDisplay_links_4_${status3.count}" href="<c:out value="${hostPath}"/>${staticAssetContextRoot}/${attachment.attachmentAsset[assetIndex].rootDirectory}/<c:out value="${attachment.attachmentAsset[assetIndex].attachmentAssetPath}"/>" target="_blank"> 
                                    <img src='<c:out value="${hostPath}"/>${staticAssetContextRoot}/${attachment.attachmentAsset[assetIndex].rootDirectory}/<c:out value="${attachment.attachmentUsage.image}"/>'
                                         alt='<c:out value="${attachment.attachmentDescription[descriptionIndex].shortDescription}"/>'
                                         border="0"  />
                                </a>
                            </c:otherwise>
...
<%-- For the Brazil store, show the "Payment Discount Promotions"--%>
	<%@ include file="ESpotNavDisplayBrazilExt.jspf" %>

</div> <%-- end of "genericESpot" class --%>

<!-- END ESpotNavDisplay.jsp -->

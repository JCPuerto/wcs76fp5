		...
		<span id="miniShopCartBody" class="body">
			<span class="text strong">
					 ...
					 <fmt:message key="NumItemsInCart" bundle="${storeText}"> 
						...
						<fmt:param><fmt:formatNumber value="${orderAmountValue}" type="currency" currencySymbol="${CurrencySymbolToFormat}" maxFractionDigits="${currencyDecimal}"/><c:out value="${CurrencySymbol}"/></fmt:param>
					 </fmt:message>
			</span> 
		</span>
									...
									<c:choose>
										<c:when test="${status.count <= countMax}">
											...
											<div class="product" id="WC_MiniShopCartDisplay_div_2_${status.count}">
												...
												<c:choose>
													<c:when test="${catEntry.dynamicKit && showDynamicKit != 'true'}">
